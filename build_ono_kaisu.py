...
**見える化ワークシートの年度ラベル**
  ...
  **令和8年度列は1か月分である。**利用者数の非ゼロ26件がすべて整数で、
  令和6・7年度が12分の1単位の小数であることと対照的である。サービス別には採れない。
...
# 介護保険法第117条第2項第1号が名指しする3サービス。
# 各年度の必要利用定員総数は計画の必須記載事項であり、努力義務ではない。
# 本町は町全域を1つの日常生活圏域とするため、圏域別の区分は生じない。
TEIIN_HOTEI = [
...
def teiin_plan(plan=None):
    """介護保険法第117条第2項第1号の必要利用定員総数のもと。

    3サービスについて、1月あたりの利用者数の見込みと町内の定員を並べて返す。
    利用者数は介護給付と予防給付の合計で、町外の事業所の利用を含む。
    戻り値は [(サービス名, 町内定員, [令和7年度実績, 令和9, 令和10, 令和11]), …]。
    """
    P = plan if plan is not None else plan_rows()
    use = {}
    for kind in ("介護", "予防"):
        for _ku, svc, _unit, ninzu, _ryo, _kyu in P[kind]:
            use[svc] = ninzu
    out = []
    for label, keys in TEIIN_HOTEI:
        tot = [0.0] * 4
        for k in keys:
            if k in use:
                tot = [a + b for a, b in zip(tot, use[k])]
        out.append((label, TEIIN.get(label, (0, "人"))[0], tot))
    return out
...
def sheet_shizentai(wb, plan):
    ...
    notes(ws, [
        ...
        "総額の置き方が同じである。ただし区分別の延ばし方は異なり、"
        "見える化は施設を利用者数一定とする。差は3年計6,678千円・月額13円**（見込量算定の再検証を参照）。",
        "**※ どちらを採るかは協議会にお諮りする。**"
...
def sheet_chiiki_ryo(wb):
    ...
    ws.append(["区分", "事業", "単位", "令和6年度", "令和7年度",
               "令和9年度", "令和10年度", "令和11年度", "出所・備考"])
    for row in chiiki_ryo():
        ku, name, unit, r6, r7, r9, r10, r11, memo = row
        f = (lambda v: None if v is None else round(v, 1))
        ws.append([ku, name, unit, f(r6), f(r7), f(r9), f(r10), f(r11), memo])
    style_header(ws)
    body(ws, wrap=(2, 3, 9))
    widths(ws, [16, 34, 26, 11, 11, 11, 11, 11, 60])
    for r in range(2, ws.max_row + 1):
        if ws.cell(r, 4).value is None:
            for c in range(4, 9):
                ws.cell(r, c).value = "【町データ待ち】" if c == 4 else None
                ws.cell(r, c).fill = WARN
        else:
            for c in range(4, 9):
                ws.cell(r, c).number_format = "#,##0.0"
    notes(ws, [
        "**※ 介護保険法第117条第2項第2号は「各年度の地域支援事業の量の見込み」を"
        "計画に定めることを求めている。**"
        "同項が別に求める費用の見込み（08_地域支援事業費）とは別の記載事項であり、"
        "努力義務ではない。",
        "※ 国保連月報から量を出せるのは介護予防・生活支援サービス事業の3事業だけで、"
        "一般介護予防事業・包括的支援事業・任意事業は町の事業実績を要する。",
        "※ 出せる3事業は、給付の見込量と同じ方法で延ばした。"
        "令和6・7年度の第1号被保険者1人当たりの件数率の平均を"
        "第10期将来推計用推計人口に乗じている。"
        "総合事業は月報が令和6年度からしか取れないため、基準期間は2年である。",
        "**※ 通いの場は、見える化では週1回以上が平成28年度以降0か所である一方、"
        "令和7年度調査では週1回以上の参加が5.5％（高齢者人口換算で約190人）ある。**"
        "量の見込みを置く前に実態の確認が要る（依頼票H3）。",
    ])


def sheet_hotei_teiin(wb, plan):
    """必要利用定員総数（介護保険法117条2項1号）。計画の必須記載事項。"""
    ws = wb.create_sheet("10_必要利用定員総数")
    ws.append(["サービス", "町内定員（令和7年度）", "令和7年度の利用（人/月）",
               "令和9年度の見込", "令和10年度の見込", "令和11年度の見込",
               "必要利用定員総数（案）", "考え方"])
    for label, cap, tot in teiin_plan(plan):
        ws.append([label, cap, round(tot[0], 1), round(tot[1], 1),
                   round(tot[2], 1), round(tot[3], 1), cap,
                   "現行定員を維持する。見込量に対し余力があり、"
                   "定員は制約として働いていない" if cap else
                   "町内に事業所がなく、整備の予定もない。0人とする"])
    style_header(ws)
    body(ws, wrap=(1, 8))
    widths(ws, [40, 15, 15, 14, 14, 14, 18, 52])
    notes(ws, [
        "※ 介護保険法第117条第2項第1号は、認知症対応型共同生活介護・"
        "地域密着型特定施設入居者生活介護・地域密着型介護老人福祉施設入所者生活介護の"
        "3サービスについて、市町村が定める区域ごとの各年度の必要利用定員総数を"
        "定めることを求めている。**努力義務ではなく必須記載事項である。**",
        "※ 本町は町全域を1つの日常生活圏域とするため、圏域別の区分は生じない。",
        "※ 利用者数は本町の被保険者が1月あたりに利用した人数で、"
        "町外の事業所の利用を含む。認知症対応型共同生活介護は介護給付と予防給付の合計。",
        "※ 町内定員は見える化システムのＤ25〜Ｄ27（令和7年度）。",
        "**※ 認知症対応型共同生活介護は、認知症基本法第18条（保健医療サービス及び"
        "福祉サービスの提供体制の整備等）とも直結する。**"
        "町内定員98人に対し本町の被保険者の利用は月45人台であり、"
        "施設・居住系サービスの自給率59.74％を踏まえると、"
        "町内の定員の相当部分が町外の方に利用されていることになる。",
        "**※ 第9期計画は「新規施設整備は原則行わず」としていたにもかかわらず、"
        "認知症対応型共同生活介護の定員は令和4年度71人から令和6年度98人へ"
        "27人増えている。増床の経緯と町外利用の割合を町に確認したうえで確定する。**",
    ])
...
def sheet_zentei(wb, plan):
    ...
    rows = [
        ...
        ("調整交付金の見込交付割合", f"{T.CHOSEI_MIKOMI * 100:.3f}％",
         "第9期計画の公表値からの逆算。第10期の全国値は国の基本指針で示される。"
         "基数は標準給付費見込額のみ（見える化システムの算式）",
...
# 地域支援事業のうち、量の見込みを月報から出せる事業と、町の実績を要する事業。
# 介護保険法第117条第2項第2号は「各年度の地域支援事業の量の見込み」を
# 計画に定めることを求めており、費用の見込み（同項の別の号）とは別の記載事項である。
CHIIKI_RYO_GEPPO = ["訪問介護相当サービス", "通所介護相当サービス",
                    "介護予防ケアマネジメント"]
CHIIKI_RYO_MACHI = [
    ("総合事業", "訪問型サービスA（緩和した基準）", "1月あたり利用者数（人）",
     "国保連月報では令和6年度以降0件。実施の有無を町に確認する"),
    ("総合事業", "通所型サービスA（緩和した基準）", "1月あたり利用者数（人）",
     "同上"),
    ("総合事業", "その他の生活支援サービス", "年間延べ利用者数（人）",
     "国保連月報では0件。配食・見守り等を総合事業で実施しているかを町に確認する"),
    ("一般介護予防事業", "介護予防普及啓発事業", "年間開催回数・延べ参加者数",
     "**町の事業実績が要る（依頼票H3）。**"
     "令和6年度の一般介護予防事業費が5,305千円と前年の5.3倍に増えており、"
     "何の事業かとあわせて確認する"),
    ("一般介護予防事業", "地域介護予防活動支援事業（通いの場）",
     "箇所数・年間延べ参加者数",
     "**見える化では週1回以上が平成28年度以降0か所である一方、"
     "令和7年度調査では週1回以上の参加が5.5％（高齢者人口換算で約190人）。"
     "実態の確認が要る**"),
    ("一般介護予防事業", "地域リハビリテーション活動支援事業", "年間派遣回数",
     "町の事業実績が要る"),
    ("包括的支援事業", "総合相談支援業務", "年間相談件数", "町の事業実績が要る"),
    ("包括的支援事業", "権利擁護業務", "年間対応件数", "町の事業実績が要る"),
    ("包括的支援事業", "包括的・継続的ケアマネジメント支援業務",
     "地域ケア会議の年間開催回数", "町の事業実績が要る"),
    ("包括的支援事業", "在宅医療・介護連携推進事業", "年間相談件数・研修回数",
     "町の事業実績が要る"),
    ("包括的支援事業", "生活支援体制整備事業",
     "生活支援コーディネーター配置数・協議体の開催回数", "町の事業実績が要る"),
    ("包括的支援事業", "認知症総合支援事業",
     "初期集中支援チームの対象者把握件数・地域支援推進員の配置数・"
     "認知症カフェの箇所数",
     "**見える化の直近値は令和5年度（訪問実績0件）・令和6年3月（推進員4人）・"
     "令和6年4月（カフェ1か所）。令和6・7年度の実績が要る（依頼票H9）**"),
    ("任意事業", "家族介護支援事業ほか", "年間実施回数・利用者数",
     "町の事業実績が要る"),
]


def chiiki_ryo():
    """地域支援事業の量の見込み（介護保険法117条2項2号）。

    費用の見込み（chiiki_plan）とは別に、法は各年度の「量」の見込みを
    計画に定めることを求めている。北塩原村の法定記載事項チェックでも
    同じ欠落が指摘されており、小規模町村の計画で落ちやすい。

    国保連月報から量を出せるのは介護予防・生活支援サービス事業の3事業だけで、
    一般介護予防事業・包括的支援事業・任意事業は町の事業実績を要する。
    出せる3事業は、給付の見込量と同じ方法（第1号被保険者1人当たりの利用率を
    基準期間の平均で置き、将来推計人口を乗じる）で延ばす。

    返り値 [(区分, 事業, 単位, 令和6年度, 令和7年度, 令和9, 令和10, 令和11, 注)]
    """
    sg = sogo_jisseki()
    pop = {y: T.POP_DAI10[y]["1号"] for y in T.PLAN_YEARS}
    _act, _sub, gen = T.extract()
    base_pop = {"令和6年度": gen["令和6年度"]["1号"],
                "令和7年度": gen["令和7年度"]["1号"]}
    rows = []
    for name in CHIIKI_RYO_GEPPO:
        k6 = sg["令和6年度"]["事業"][name][0]
        k7 = sg["令和7年度"]["事業"][name][0]
        rate = ((k6 / base_pop["令和6年度"]) + (k7 / base_pop["令和7年度"])) / 2
        est = [rate * pop[y] for y in T.PLAN_YEARS]
        if name == "介護予防ケアマネジメント":
            unit, div = "年間件数", 1
        else:
            unit, div = "1月あたり利用者数（人）", 12
        rows.append(("総合事業", name, unit, k6 / div, k7 / div,
                     est[0] / div, est[1] / div, est[2] / div,
                     "国保連月報の総合事業コード。"
                     "令和6・7年度の第1号被保険者1人当たりの件数率の平均を"
                     "第10期将来推計用推計人口に乗じた"))
    for ku, name, unit, note_ in CHIIKI_RYO_MACHI:
        rows.append((ku, name, unit, None, None, None, None, None, note_))
    return rows
