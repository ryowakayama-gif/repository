...
# 逆算される調整交付金見込額そのものは
#   （3,566,486＋207,260）×23％＋（3,566,486＋110,960）×5％－832,562＝219,272千円
# で、基数の取り方によらない。割合はこれを基数で割ったものであり、
#   標準給付費見込額のみを基数とすると 219,272÷3,566,486＝6.148％
#   総合事業費を含めると             219,272÷3,677,446＝5.963％
# となる。見える化システムは前者（調整交付金見込額＝標準給付費見込額×割合）を用いる。
# 調整交付金相当額（5％）の基数に総合事業費が入るのとは非対称だが、
# 川崎町の第9期将来推計総括表（システム出力）の算式がそうなっている。
# なお第10期の割合はシステムが全国の仮置値から自動計算するため、
# ここでの6.148％は町が画面で確認するまでの仮置きである。
CHOSEI_MIKOMI = 0.061481
