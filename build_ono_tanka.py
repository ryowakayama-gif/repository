...
# 認定者数の見込。年齢階層別の認定率（令和7年度実績。年報 様式1の5の認定者数を
# 様式1の年齢階層別第1号被保険者数で割ったもの）を将来の年齢階層別人口に乗じる。
# 03_認定者推計（build_ono_santei.py）と同じ方法である。
#
# **令和6年度→令和7年度の認定率の変化を一定として延ばす置き方は採れない。**
# 65〜74歳が▲1.38ポイント、75〜84歳が▲1.66ポイント、85歳以上が▲11.20ポイントで、
# これを毎年加えると令和10年度に65〜74歳の認定率が負になり、
# 令和11年度の認定者数は329人まで落ちる。令和7年9月の断層がこの変化に入っているためで、
# 断層の原因が確認できるまでは変化0（令和7年度の水準で据え置き）とする。
NINTEI_RATE_R7 = {          # 令和7年度の年齢階層別認定率（第1号・年度末）
    "65-74": 64 / 1634,
    "75-84": 204 / 1090,
    "85+": 515 / 719,
}
NINTEI_RATE_R6 = {          # 参考：令和6年度。差を延ばすと破綻することの根拠
    "65-74": 89 / 1680,
    "75-84": 214 / 1050,
    "85+": 632 / 763,
}
NINTEI_2GO = 9              # 第2号被保険者の認定者数（令和8年3月末実績・年報 様式1の5）
# NINTEI_EST は POP_DAI10 の定義後に算出する（ファイル末尾）。
...
#
# 令和7年度単年に切り替える基準（『見込量算定の手引き』手順⑦「伸びの置き方」）
#   ① 令和7年度の件数が令和5〜7年度の3年平均から5％以上離れていること（趨勢がある）
#   ② 令和8年度の国保連月報の前年同期比（審査202605〜202607と前年の同じ月）が
#      その向きを裏づけること（増なら1.05以上、減なら0.95以下）
# 2つそろったものだけを切り替える。①だけのサービス（訪問介護・介護予防訪問看護・
# 地域密着型介護老人福祉施設入所者生活介護・小規模多機能型居宅介護など）は、
# 令和8年度に入って向きが反転しているため3年平均のまま置く。
# この判定は build_ono_kaisu.check_base() が月報から再計算し、下表とずれたら知らせる。
BASE_OVERRIDE = {
    ...
    ("訪問看護", "介護"):
        (["令和7年度"], "令和5年度456件→令和6年度470件→令和7年度519件。"
                    "令和7年度は3年平均を7.7％上回り、令和8年度の前年同期比も1.120。"
                    "増加が続いているため直近年度で置く"),
    ("通所リハビリテーション", "予防"):
        (["令和7年度"], "令和5年度280件→令和6年度278件→令和7年度329件。"
                    "令和7年度は3年平均を11.1％上回り、令和8年度の前年同期比も1.297。"
                    "増加が続いているため直近年度で置く"),
    ("福祉用具貸与", "予防"):
        (["令和7年度"], "令和5年度605件→令和6年度607件→令和7年度688件。"
                    "令和7年度は3年平均を8.7％上回り、令和8年度の前年同期比も1.204。"
                    "増加が続いているため直近年度で置く"),
    ("居宅介護支援・介護予防支援", "予防"):
        (["令和7年度"], "令和5年度802件→令和6年度807件→令和7年度919件。"
                    "令和7年度は3年平均を9.0％上回り、令和8年度の前年同期比も1.206。"
                    "事業対象者が令和6年度4件から令和7年度30件へ増えたことと整合する"),
    ("認知症対応型共同生活介護", "介護"):
        (["令和7年度"], "令和5年度611件→令和6年度609件→令和7年度537件。"
                    "令和7年度は3年平均を8.4％下回り、令和8年度の前年同期比も0.903。"
                    "減少が続いているため直近年度で置く"),
    ("認知症対応型共同生活介護", "予防"):
        (["令和7年度"], "令和6年度37件から令和7年度14件へ。"
                    "令和8年度の前年同期比も0.750。介護のグループホームへの移行を含む"),
}
...
# 所得段階別第1号被保険者数（年報 様式1 所得段階別・令和7年度末）と2種類の割合。
#   標準割合＝介護保険法施行令第38条の標準段階の割合。**保険料基準額の算定に用いる。**
#   賦課割合＝実際に賦課する割合。第1〜3段階は公費による低所得者軽減強化を反映して
#            0.285／0.485／0.685となる。
#            年報 様式1 所得段階別の令和7年度は第2段階が486となっているが、
#            令和6年度は485であり、第9期計画本文も「基準額×0.485」
#            （年額38,500円＝79,200円×0.485の100円未満切上げ）である。
#            令和7年度の486は年報の入力誤りとみて485を採る。
# 基準額の算定で軽減前の標準割合を用いるのは、軽減相当額が別枠の公費
# （国1/2・県1/4・町1/4）で補填され、保険料収納必要額から控除されないことによる。
# 見える化システムの「①標準段階区分・割合を用いる場合の入力」の画面でも
# 0.4550／0.6850／0.6900が固定で表示される（令和8年9月25日に画面で確認）。
# 第9期計画の補正後被保険者数10,136人／第1号被保険者数10,409人＝0.974も、
# 軽減前の標準割合でしか再現できない（軽減後だと0.93台にしかならない）。
SHOTOKU_R7 = [
    # 段階名, 標準割合（算定用）, 賦課割合（軽減後）, 令和7年度末の人数
    ("第1段階", 0.455, 0.285, 525), ("第2段階", 0.685, 0.485, 297),
    ("第3段階", 0.690, 0.685, 272), ("第4段階", 0.900, 0.900, 434),
    ("第5段階", 1.000, 1.000, 610), ("第6段階", 1.200, 1.200, 491),
    ("第7段階", 1.300, 1.300, 418), ("第8段階", 1.500, 1.500, 231),
    ("第9段階", 1.700, 1.700, 77), ("第10段階", 1.900, 1.900, 36),
    ("第11段階", 2.100, 2.100, 14), ("第12段階", 2.300, 2.300, 7),
    ("第13段階", 2.400, 2.400, 31)]
...
# 後期高齢者加入割合補正係数。受領した見える化ワークシート「5_保険料推計」96行。
# 第10期の全国値が確定すると動くため、町が画面で確認するまでの値である。
KOKI_HOSEI = {"令和9年度": 0.9558, "令和10年度": 0.9802, "令和11年度": 1.0065}
...
DAI9_HOSEI = 10136        # 同 所得段階別加入割合補正後被保険者数（3年計・人）
DAI9_POP3 = 10409         # 同 第1号被保険者数（3年計・人）


def shotoku_hosei():
    """所得段階別加入割合補正係数（＝標準割合の加重平均）。

    令和7年度末の段階別構成比（年報 様式1 所得段階別）による。
    """
    return (sum(hyo * n for _nm, hyo, _fu, n in SHOTOKU_R7)
            / sum(n for *_x, n in SHOTOKU_R7))
...
def hosei_ninzu(pop3):
    ...
    割合は軽減前の標準割合（SHOTOKU_R7 の2列目）を用いる。
    """
    act_tot = sum(n for *_x, n in SHOTOKU_R7)
    return sum(hyo * round(n / act_tot * pop3) for _nm, hyo, _fu, n in SHOTOKU_R7)


def chosei_wari(year=None):
    """調整交付金見込交付割合。年度を省くと第10期3年の単純平均を返す。

    ＝（第1号被保険者負担割合＋5％）
      －第1号被保険者負担割合×後期高齢者加入割合補正係数×所得段階別加入割合補正係数

    第9期計画の公表値で検算すると、調整交付金見込額216,070千円÷標準給付費見込額
    3,566,486千円＝6.058％。上の算式に同計画の補正係数0.974を入れると
    後期高齢者加入割合補正係数は0.980となり、整合する。
    """
    if year is None:
        return sum(chosei_wari(y) for y in KOKI_HOSEI) / len(KOKI_HOSEI)
    return (FUTAN_WARIAI + CHOSEI_SOTO
            - FUTAN_WARIAI * KOKI_HOSEI[year] * shotoku_hosei())


# 第10期3年の単純平均。年度別の割合を標準給付費で加重しても差は0.002ポイントで、
# 保険料月額では1円に満たない。
CHOSEI_MIKOMI = chosei_wari()
...
def sheet_kando(wb, act, gen, cases):
    ...
    notes(ws, [
        ...
        "**※ 見える化システムの見込量算定は「第1号被保険者数×認定率×利用率」の3段で、"
        "利用率の分母は認定者数（在宅サービスは認定者数−施設・居住系利用者数）である。**"
        "本表の「第1号被保険者1人当たり」はこの3段と代数的に同じもので、"
        "差は将来の認定率÷基準期間の認定率だけである。",
        "**※ 小野町の認定者数は令和7年9月審査分に段差がある（961人→862人）。"
        "国保連月報で28か月を並べると、段差は認定者数にだけ現れ受給者数は連続している"
        "（認定者▲11.4％に対し受給者計▲3.9％、利用率＋8.4％）。**"
        "基準期間の令和5〜7年度はこの段差をまたぐため、"
        "認定率を3段に分解して使うと10.5％低く出る。"
        "段差の後にそろえて比べれば差は1.0％にとどまる。"
        "計上方法が確認できるまで、受給率のまま置く。",
    ])
    return ws
...
def sheet_hyojun(wb, act, gen, proj):
    ...
    notes(ws, [
        ...
        f"見込交付割合は第9期計画の公表値から逆算した小野町の実績値{wari * 100:.3f}％を置いた。"
        "**小野町は後期高齢者加入割合が高く低所得層が厚いため、標準の5％を上回る。**"
        "第10期の値は見える化システムの係数シート（全国値）で確認する。1ptで月額295円。",
        "※ 補正後被保険者数は、令和7年度末の所得段階別構成比（様式1 所得段階別）を"
        f"第10期3年分の第1号被保険者数{pop3:,}人に当てたもの。所得段階の設定が変われば動く。",
...
def nintei_est(rate=None, years=None):
    """年齢階層別認定率×将来推計人口による第1号被保険者の認定者数の見込み。

    rate を渡さなければ令和7年度の実績の認定率（変化0）を用いる。
    """
    rate = rate or NINTEI_RATE_R7
    return {y: round(sum(rate[k] * POP_DAI10[y][k] for k in rate))
            for y in (years or EST_YEARS)}


NINTEI_EST = nintei_est()
...
def doc_dai9(gen, proj):
    ...
    table(doc, ["区分", "第9期の見込み（人／月）", "令和7年度実績（人／月）",
        ...
        ("地域密着型介護老人福祉施設", "―", f"{496 / 12:.1f}", "―", "126,350",
         "町内施設。**定員は令和2年度に29人から58人へ増えており、"
         "受給者41.3人／月は定員内である**（見える化Ｄ25）"),
    ])
    doc.add_paragraph(
        "**町内の定員は、介護老人福祉施設54人、地域密着型介護老人福祉施設58人、"
        "認知症対応型共同生活介護98人である**（見える化システムＤ25・Ｄ26、令和7年度）。"
        "介護老人福祉施設は受給者68.9人／月が定員54人を14.9人上回っており、"
        "この差が町外の施設の利用にあたる。介護老人保健施設は町内に施設がなく、"
        "受給者28.6人／月はすべて町外である。")
