...
check('ブリッジに消費税の項目がない（両者とも税込）', '消費税相当' not in doc)
V_S, V_A = 272275, 252955
U_S, U_A = 47238 * 1000 / V_S, 44645 * 1000 / V_A
check('ブリッジ 水量差 ▲3,352千円',
      has('3,352') and round((V_A - V_S) * U_S / 1000) == -3352)
check('ブリッジ 単価差 +759千円', has('759') and round((U_A - U_S) * V_A / 1000) == 759)
check('ブリッジ 予算の保守的計上 ▲2,739千円', has('2,739') and 44645 - 41906 == 2739)
check('ブリッジ 各段階の残高', has('47,537', '47,238', '43,886', '44,645'))
check('ブリッジ 合計が整合', round(52273 - 4736 - 299 - 3352 + 759 - 2739) == 41906)
check('中間点44,645がR7決算の税込換算と一致', round(40586357 * 1.1 / 1000) == 44645)
...
check('決算と並べる際は1.1で除する旨を明記', has('1.1で除する'))
...
check('増収試算の基礎3種を提示', has('41,906', '44,579', '44,645'))
for b, a, g in [(41906, 46067, 4161), (44578.708, 49005, 4427), (44645, 49078, 4433)]:
    check('基礎%s → 改定後%d・増収%d' % (format(round(b), ','), a, g),
          has(format(a, ','), format(g, ',')) and round(b * 1.0993) == a and round(b * 0.0993) == g)
check('年間全件調定と決算税込換算が0.15%一致',
      has('0.15') and abs(44578.708 / 44645 - 1) < 0.0016)
check('改定後も経営戦略に3,268千円届かない旨を明記',
      has('3,268') and 52273 - 49005 == 3268)
...
check('41,906千円が税込である旨を明記', has('合算・税込'))
...
check('予算編成が決算確定前である旨を明記', has('より前に編成'))
...
check('Excelに31シートある', len(wb.sheetnames) == 31, '検出 %d' % len(wb.sheetnames))
...
check('Excel 26 にブリッジ各段階がある', {52273, 47537, 47238, 43886, 44645, 41906} <= n26)
check('Excel 26 に増減額がある', {-4736, -299, -3352, 759, -2739} <= n26)
check('Excel 26 に税込である証明がある', {43174, 39249, 39250, 43836} <= n26)
check('Excel 26 にセグメント内訳がある', {34356, 7550, 41906, 119, 42025} <= n26)
