...
check('表1〜表18の見出しがある', all('表%d' % i in doc for i in range(1, 19)))
check('章立て1〜10＋参考がある', all(h in doc for h in
      ['本資料の趣旨', '現行使用料体系の構造', '単価差の縮小余地', '改善レンジと単価差の縮小水準',
       '2案の比較と使用量別の請求影響', '平年度増収額と経費回収率', '段階的な縮小の考え方',
...
check('表15 第2段階の平年度増収 8,860,468円', round(t2 - ANNUAL) == 8860468,
      '再計算 %s' % format(round(t2 - ANNUAL), ','))
check('表15 第2段階の平均増収率 +19.88%', has('+19.88') and abs((t2 / ANNUAL - 1) * 100 - 19.88) < 0.006)
check('表15 第2段階の経費回収率 59.3%', has('59.3') and abs(49.5 * t2 / ANNUAL - 59.3) < 0.06)
...
for v, cur, s1, s2v, d1, d2 in [(5, 2217, 2217, 2217, 0.0, 0.0), (10, 2624, 3020, 3416, 15.1, 30.2),
    ...
    check('表16 月%d㎥ 第1段階%s円 第2段階%s円' % (v, format(s1, ','), format(s2v, ',')),
...
check('表13から事業別経費回収率の列を外し別添に送っている',
...
check('表14の増収額が決算ベース（40,586千円×平均増収率）で算定されている',
...
check('表14が表13と基礎の異なる旨を注記している',
      has('調定分布から積み上げた表13の税抜増収額（A案4,074千円等）とは基礎が異なる')
