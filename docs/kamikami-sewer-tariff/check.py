...
check('R7決算 下水道使用料 40,586,357円', has('40,586,357'))
check('R7決算との推計差 0.15%', has('0.15') and abs(40526099 / 40586357 - 1 + 0.0015) < 0.0001)
check('セグメント別 公共33,793,885円・漁集6,792,472円', has('33,793,885', '6,792,472'))
check('セグメント分離の検算が成立', 40586357 - 6792472 == 33793885 and 34359685 - 33793885 == 565800)
check('R7有収水量 公共210,398㎥・漁集42,557㎥', has('210,398', '42,557'))
check('経費回収率 R7 49.5%・R6 47.0%', has('49.5', '47.0'))
check('経費回収率の定義が32表と一致', abs(40881245 / 87078000 * 100 - 47.0) < 0.15)
check('経営戦略の経費回収率は資本費を含む旨を明記', has('資本費を含む分母'))
check('R8予算のR7決算比 +3.3%', has('+3.3') and abs(41906 / 40586 - 1 - 0.033) < 0.001)
check('残差の内訳 ▲3,047／+690／+1,320', has('3,047', '690', '1,320'))
check('残差の内訳が合計と一致', abs(-3047 + 690 + 1320 - (-1037)) <= 1)
check('水量予測の過大 +5.8% を明記', has('+5.8') and abs(222618 / 210398 - 1 - 0.058) < 0.001)
...
check('予算額の前提（合算・税抜）を明記', has('合算・税抜'))
check('1会計2セグメントである旨を明記', has('1会計2セグメント'))
check('R8当初予算書での確認が必要な旨を明記', has('令和8年度当初予算書'))
check('R7決算書に41,906千円が現れない旨を明記', has('令和7年度決算書には現れない'))
check('R7当初予算 営業収益 43,836千円を明記', has('43,836'))
...
check('Excelに30シートある', len(wb.sheetnames) == 30, '検出 %d' % len(wb.sheetnames))
...
check('Excel 26 に残差内訳がある', {-3047, 690, 1320, 39896, 40586} <= n26)
