...
# 繰出基準：使用料対象資本費＝max(0, 使用料収入−汚水維持管理費)
R7_FEE = 40586357 / 1.1 / 1000        # 税抜・千円（40,586千円は税抜の決算値）
FEE7 = 40586.357
OM7 = 81993.0                          # 汚水維持管理費（千円・税抜）
check('表14 繰入金の扱いによる2ケースを併記',
      has('繰入金の扱いによる経常収支比率への効果の違い')
      and has('繰入金を据え置く場合') and has('増収分だけ基準外繰入金を減額する場合'))
check('R7は使用料対象資本費がゼロ（使用料 < 汚水維持管理費）',
      max(0.0, FEE7 - OM7) == 0.0 and has('使用料対象資本費はゼロ'))
check('汚水維持管理費を超えるには約2.02倍が必要',
      has('2.02倍') and abs(OM7 / FEE7 - 2.02) < 0.005,
      '再計算 %.3f倍' % (OM7 / FEE7))
check('+15%でも46,662千円にとどまる旨が整合',
      has('46,662千円') and round(FEE7 * 1.1497) == 46662,
      '再計算 %s千円' % format(round(FEE7 * 1.1497), ','))
check('事業別の必要倍率 公共2.00倍・漁集2.89倍を明記', has('公共2.00倍', '漁業集落排水2.89倍'))
check('5〜15%では基準内繰入金が減額されない旨を明記',
      has('基準内繰入金は減額されず、増収分はそのまま経常収益の増加となる'))
