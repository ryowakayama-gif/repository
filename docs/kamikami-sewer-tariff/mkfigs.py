...
PLANS = {
    '縮小なし':  (1008, 37, 215, 220),
    'A案':      (1008, 55, 203, 220),
    'B案':      (1008, 73, 191, 220),
    '強い縮小':  (1008, 100, 173, 220),
}
...
def fig1():
    ...
    pts = [(v, RC.month_net(v) * 1.1 / v) for v in range(5, 51)]
    ...
    out.append(txt(X(10) + 14, Y(131.23) + 20, '全区分で最も安い', 12, MUTED))
...
def fig3():
    ...
    for v in range(5, 51, 5):
        out.append(txt(X(v), H - B + 22, str(v), 12, MUTED, 'middle'))
    styles = [('縮小なし', GRAY, '7 5', 2), ('A案', RAMP['弱'], '2 4', 3),
              ('B案', RAMP['中'], None, 3.5), ('強い縮小', RAMP['強'], None, 2.5)]
...
# ---------- 図4 段階的な縮小で単価の開きが縮まる ----------
def fig4():
    groups = [('現行', 37, 174, '4.70倍'), ('第1段階（B案）', 73, 191, '2.62倍'),
              ('第2段階（参考）', 109, 207, '1.90倍')]
