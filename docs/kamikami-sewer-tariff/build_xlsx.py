...
SHEETS = [
    ...
    ('34_一律改定との比較',      '34_一律改定との比較.csv'),
    ('35_基本使用料の改定',      '35_基本使用料を改定する場合.csv'),
    ('36_料金区分の変更',        '36_料金区分を変える場合.csv'),
    ('37_基本水量の感度',        '37_基本水量を下げる場合の感度.csv'),
]
...
rows = [
    ...
    ('31_決算統計の税区分', '★説明資料 表16・表19の根拠。作成要領による各表の税区分と経営指標への反映'),
    ('32_料金収入算定の精査', '★説明資料 表2・表3・表8の根拠。区分別シェアの分母と算定モデルの精査結果'),
    ('33_条例との整合確認', '★説明資料 表22の根拠。条例別表・第17条・第18条との機械的な整合確認'),
    ('34_一律改定との比較', '★説明資料 表9〜表12の根拠。全体改定率での試算と構造見直し案との比較'),
    ('35_基本使用料の改定', '★説明資料 表13・表14の根拠。基本使用料を動かした場合の増収と負担配分'),
    ('36_料金区分の変更', '★説明資料 表15・表16の根拠。区分数・境界水量・基本水量を変えた7案の比較'),
    ('37_基本水量の感度', '★基本水量を下げる場合、基本水量内1,934レコードの想定水量による幅'),
    ('', ''),
