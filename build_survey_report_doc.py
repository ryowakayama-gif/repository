...
MBAND("f_iji", "在宅生活の維持の見通しと就労継続（n=98）",
      [("在宅生活の維持が困難", [72, 26]),
       ("介護者の就労継続が困難", [41, 57])],
      ["該当する", "該当しない"], ncol=2)
P("本調査で抽出された課題事例98人のうち、"
  "現在の状態では在宅生活の維持が困難とされた方は72人（73.5%）です。"
...
NOTE("事業所票15件のうち1件は、事業所名の記入はあるものの"
     "各設問が0又は記号のみとなっています。"
     "対象となる事例が0件であったのか未記入であるのかが判別できないため、"
     "「回答事業所15件」と「集計に反映した事業所」を"
     "区別する必要があります。提出元への確認を要します。")
...
NOTE("本章では、施設・居住系サービスを提供する事業所18件にご回答いただいた"
     ...
     "受けないもの12）で、回答は18施設、未回答は13施設です（把握率58.1%）。"
     "本章の数値は回答した18施設の記述値であり、区域全体の値ではありません。"
     "定員は北海道が公表する名簿によります（第７章第１節）。")
TBL(["区分", "区域内", "回答", "未回答", "把握率"],
    [["特別養護老人ホーム", "3", "2", "1", "66.7%"],
     ["地域密着型介護老人福祉施設", "3", "2", "1", "66.7%"],
     ["介護老人保健施設", "3", "3", "0", "100.0%"],
     ["認知症対応型共同生活介護", "7", "5", "2", "71.4%"],
     ["特定施設入居者生活介護", "3", "2", "1", "66.7%"],
     ["　指定を受けるもの　小計", "19", "14", "5", "73.7%"],
     ["住宅型有料老人ホーム", "9", "3", "6", "33.3%"],
     ["サービス付き高齢者向け住宅", "2", "1", "1", "50.0%"],
     ["軽費老人ホーム", "1", "0", "1", "0.0%"],
     ["　指定を受けないもの　小計", "12", "4", "8", "33.3%"],
     ["合計", "31", "18", "13", "58.1%"]],
    [6.0, 2.4, 2.4, 2.4, 2.6])
NOTE("未回答13施設のうち8施設は介護保険の指定を受けない住まいです。"
     "指定を受けるものでは、"
     "東神楽町特別養護老人ホームアゼリアハイツ（広域型・ユニット型の2件）、"
     "グループホームファミリー、グループホームくるみの郷、"
     "さわやか東神楽館（特定施設・区域内最大）が未回答です。"
     "定員による把握率は、指定を受けるものが61.1%（717人中438人）です。")
...
rows = [["採用者数", "%d〜%d" % (SAIYO, SAIYO + 7),
         "%.1f〜%.1f%%" % (SAIYO / (N_SIS + N_HOU) * 100,
                           (SAIYO + 7) / N_ALL * 100)],
        ["離職者数", "%d〜%d" % (RISHOKU, RISHOKU + 2),
         "%.1f〜%.1f%%" % (RISHOKU / (N_SIS + N_HOU) * 100,
                           (RISHOKU + 2) / N_ALL * 100)],
        ["差（採用－離職）", "%+d〜%+d" % (SAIYO - RISHOKU,
                                        SAIYO + 7 - RISHOKU - 2), "―"]]
TBL(["区分", "人数", "介護職員に対する割合"], rows, [6.0, 3.4, 4.4])
GBAR("f_saiyo", "令和6年度の採用者数と離職者数（重複を除く）",
     ["採用者数", "離職者数"],
     [("人数", [SAIYO, RISHOKU])], unit="人", ncol=1, ylabel="人")
P("令和６年度の採用者数は83〜90人、離職者数は65〜67人で、"
  "差は＋18人から＋23人です。"
  "採用が離職を上回っていますが、"
  "介護職員数に対する離職者の割合は18.0〜18.7%であり、"
...
P("介護職員は348〜361人です（重複の取扱いにより変わります）。"
  ...
  "一方、勤続１年未満が19.9%、令和６年度の離職者が65〜67人であり、"
  "毎年２割近くが入れ替わっています。"
  "60歳以上の職員は13.3%です。"
  "勤務継続の意向は把握していないため、"
  "将来の退職見込みは別途確認を要します。")
...
P("④ 居所変更実態調査は、区域内31施設のうち13施設が未回答です"
  ...
  "定員による把握率は、指定を受ける施設・居住系で61.1%です。")
