...
def steps(plan):
    ...
    return [
        ...
        ("3", "基準年度の決定", "完結した最新年度を基準年度とする", "済",
         ...
         "**第9期でも同じことが起きていた。**第9期の見込みは3か年の振れ幅が"
         "0.42％しかなく自然体推計そのものであり、その水準は令和5年度の実績より"
         "7.3％高い。実績／見込みが令和6年度98.6％・令和7年度91.1％と"
         "なった理由がこれで説明できる。"
         "**見える化の令和8年度も月報の実績より8.0％高く、同じ幅である**"
         "（見込量算定の再検証 第2・3章）"),
        ...
        ("6", "パターンの列挙", "基準年度・出所・伸びの組合せを並べる", "済",
         ...
         "**令和8年度の置き方は5通りを比べた。**月報の実績0.9598と、"
         "月報を使わない置き方（認定者数の伸び＋サービス別趨勢の半分）0.9694が"
         "1.0％差で揃う一方、現行方式1.0171はどちらからも離れる。"
         "**延ばし方を見える化の仕様にそろえた（9月25日）。**"
         "施設は利用者数一定、居住系・在宅は認定者数で延ばす。"
         "従前の置き方（第1号被保険者数で一律）との差は"
         "3年計6,678千円・月額13円"),
        ...
        ("14", "確認事項の整理", "確定していない事項を一覧にする", "済",
         ...
         "**厚生労働省の保険者別配布シート（標準の推計方法を当てはめた誤差。"
         "基本推計と包括推計の別）を依頼に追加した。**"
         "これがあれば推計手法の選択の妥当性を国のデータで確かめられる"),
        ("付", "論点の重みの基準", "総給付費1％が保険料の月額で何円かを先に出す",
         "今回",
         "**総給付費1％＝保険料の月額58.0円。**"
         "意思決定に効く論点と誤差の範囲の論点を分けるための基準。"
         "見える化の仕様との差13円は後者、令和8年度の置き方の差347円は前者にあたる"),
        ("付", "上限1.1・下限0.9", "国のガイドの制約に収まるかを確かめる", "今回",
         "**認定率・利用率・1人1月あたり回（日）数・1人1月あたり給付費の"
         "4項目とも適合。**最も振れるのはサービス利用率で1.00％"),
    ]
