...
def steps(plan):
    ...
    return [
        ...
        ("付", "推計手法の妥当性", "基本推計と包括推計のどちらが当たるかを確かめる",
         "今回",
         "**国のデータ（見える化「推計パターン毎の乖離状況」）で検証した。**"
         "認定者数の誤差は令和6年度▲3.10％・令和7年度＋3.23％（基本推計）で、"
         "全国1,267位・1,015位／約1,570保険者と中位より下。"
         "**サービス別では誤差10％超が10件あり、"
         "標準の推計方法は小野町では当たらない。**"
         "当方が実績を基点として総額を月報で裏づけていることの裏づけになる"),
        ...
        ("14", "確認事項の整理", "確定していない事項を一覧にする", "済",
         ...
         "**9月25日に、見える化システムの「推計パターン毎の乖離状況」から"
         "推計手法の妥当性を検証した（推計手法の妥当性検証）。**"
         "基本推計と包括推計の優劣は年度で逆転し、2年平均では基本推計が"
         "わずかに優る。現行の設定を変える理由はない"),
