...
3 出所により水準が異なる。
  ...
  給付費は令和6年度から令和8年度まで3町のファイルの合計と
  広域連合単位のファイルが円単位で一致する（確認事項No.87は解決した）。
...
PATTERNS = [
    ...
    dict(no="P7", kijun="令和7年度", demae="計画作成支援ツール（3町合計）",
         ...
         riyu="3町合計は利用者数において広域連合単位を大きく上回る"
              "（区域外の施設を利用する被保険者と区域内の施設を利用する"
              "他保険者の被保険者の扱いが違う）。"
...
for i, (k, v, b) in enumerate([
    ...
    ("出所により水準が異なる",
     ...
     "給付費は令和6年度から令和8年度まで3町のファイルの合計と"
     "広域連合単位のファイルが円単位で一致します（確認事項No.87は解決）。",
     "02シート"),
    ("総括表は内的整合が取れている",
     "利用者数×1人あたり給付費＝給付費が全行で成立し、"
...
r = note(ws, r,
         ...
         "注2）3町のファイルの合計と広域連合単位のファイルは、"
         "令和6年度から令和8年度まで円単位で一致します。"
         "令和7年度にあった129,580,267円の差は、"
         "令和8年9月25日に再提供を受けたファイルにより解消しました"
         "（確認事項No.87）。"
...
for src, v6, v7, tan, naiyo in [
    ...
    ("見える化システム 総括表詳細（５）の合計", R6_TSUMI, R7_TSUMI, "広域連合",
     "利用者数×1人あたり給付費の積上げ"),
    ("給付費データ集計（広域連合単位のファイル）",
     KJ.KYUFU_TOTAL[("大雪", "R6")][-1], KJ.KYUFU_TOTAL[("大雪", "R7")][-1],
     "広域連合",
     "「給付費総額」（保険請求額＋保険利用者負担額）の合計であり"
     "費用額に当たる。年報の給付費とは範囲が異なる"),
    ("給付費データ集計（3町のファイルの合計）",
     sum(KJ.KYUFU_TOTAL[(t, "R6")][-1] for t in TOWNS3),
     sum(KJ.KYUFU_TOTAL[(t, "R7")][-1] for t in TOWNS3), "3町合計",
     "令和6年度から令和8年度まで広域連合単位のファイルと完全に一致する。"
     "令和7年度にあった%s円の差は、令和8年9月25日に再提供を受けた"
     "ファイルにより解消した（確認事項No.87）"
    ...
    r = body(ws, r, [src, yen(v6) if v6 else "―", yen(v7) if v7 else "―",
                     tan, "", "", naiyo], height=32)
r = note(ws, r,
         "注）給付費データ集計の給付費（給付費総額）の合計は、"
         "令和6年度において決算の保険給付費（款2）に"
         "介護予防・生活支援サービス事業費を加えた額と%s円の差で一致します。"
         "給付費データの給付費総額は総合事業費を含む範囲であるとみられます。"
         "［要確認］"
...
# 3町合計と広域連合単位はいずれの年度も一致する（確認事項No.87は解決）。
_gap = {y: KJ.sa(y) for y in ("R6", "R7", "R8")}
chk(11, "3町合計と広域連合単位の給付費がいずれの年度も一致すること",
    "R6 %s円／R7 %s円／R8 %s円" % tuple(yen(_gap[y]) for y in
                                    ("R6", "R7", "R8")),
    "3か年とも円単位で一致（令和7年度の差は再提供により解消した）",
    all(_gap[y] == 0 for y in ("R6", "R7", "R8")))
...
for i, (k, v) in enumerate([
    ("令和8年度の月数", "公表されていないため、給付費の比から推計するにとどめた。"
     "［要確認］"),
    ("給付費データの給付費総額と年報の給付費の範囲の違い",
     "給付費総額は保険請求額＋保険利用者負担額であり費用額に当たる。"
     "本表の算定は年報及び総括表（保険給付費）によっている"),
