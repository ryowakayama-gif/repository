...
GENSEN = [
    ("①", "地域包括ケア「見える化」システム",
     "厚生労働省",
     "令和8年7月22日・7月29日・7月30日取得、令和8年8月31日受領",
     "A系列（人口）・B系列（認定）・C1（保険料）・D系列（受給）・"
     "K系列（事業所数）・M系列（従事者数）・W144（交付金評価指標）・"
     "δ（介護サービス自給率）。"
     "令和8年8月31日に総括表（6シート）及び管理指標グラフ（7指標）を受領し、"
     "第7期から第9期までの計画値・実績値・対計画比を追加した",
     "E1"),
    ...
    ("④", "日本の地域別将来推計人口（令和5年推計）",
     ...
     "E1"),
    ("⑤", "介護保険事業状況報告", "厚生労働省",
     "各年3月末現在。年報（令和6・7年度）及び月報（令和8年4〜7月）は"
     "令和8年8月28日受領",
     "要介護認定者数・受給者数。"
     "年報により給付費・保険料収納率・所得段階別被保険者数・"
     "介護給付費準備基金の残高を確定した",
     "E1"),
    ("⑥", "介護保険事業所一覧", "北海道",
    ...
    ("⑫", "④2025年健康とくらしの調査（JAGES）",
     ...
     "令和7年度",
     "3町の回答4,798票（回収率67.4％）。"
     "うち個票4,729票を令和8年8月に受領しクロス集計・分析に用いた。"
     "個人情報のため本集には収録せず、"
     "成果品「調査クロス集計・分析」に集計結果を収めている",
     "―"),
    ("⑬", "介護予防・日常生活支援総合事業（地域支援事業）の"
     "実施状況に関する調査", "厚生労働省",
     "令和6年度実績・令和8年8月28日受領",
     "市町村Ⅰ〜Ⅲの3表。通いの場の展開状況（箇所数・参加者実人数・"
     "運営主体・開催頻度）、一般介護予防事業、サービス・活動事業。"
     "構成3町ごとの回答",
     "―"),
    ("⑭", "介護保険特別会計歳入歳出決算書", "大雪地区広域連合",
     "令和6年度・令和8年8月28日受領",
     "款項別の決算額、実質収支額、介護給付費準備基金の推移。"
     "介護保険事業状況報告年報の様式4と22項目すべて一致する",
     "―"),
    ("⑮", "介護保険事業財政調整基金条例・"
     "介護保険料の減額賦課に伴う処遇改善臨時特例基金条例",
     "大雪地区広域連合", "令和8年8月28日受領",
     "基金の設置・積立て・処分の要件。"
     "第5条第3項の積立ての上限（計画期間の保険料収納必要額の100分の10）",
     "―"),
]
...
def build_index(files):
    ...
    r = head(ws, r, ["収録ZIP", "ファイル", "収録した内容", "主な出典",
                     "シート数"])
    for a in files:
    ...
    for a in [
        ...
        ("④", "④健康とくらしの調査の個票（4,729票）",
         "個人情報のため。"
         "令和8年8月に受領し、クロス集計・分析（成果品「調査クロス集計・分析」"
         "24シート）に用いているが、本集及び成果品には集計値のみを収める。"
         "個票は受託者の作業環境にのみ保持し、"
         "リポジトリ及び納品データには格納していない。", "", ""),
    ...
    note(ws, r + 1,
         ...
         "注3）本集の各表は、成果品と同じデータモジュールから"
         "機械的に書き出している。"
         "したがって本集に収めた表と成果品の該当箇所との間に"
         "転記の誤りは生じない。"
         "ただしこれは「本集に収めた範囲」についての保証であり、"
         "成果品のすべての数値を保証するものではない。"
         "本集に収めていない数値"
         "（④健康とくらしの調査の集計値、"
         "見える化δ、事業所調査の照会中の項目など）は、"
         "各成果品の出典欄により確認いただきたい。", 5)
