...
MIRU = [
    ...
    ("経年比較の出所",
     ...
     "計画素案 第2章第4節の表から読む。"
     "①②③の内容の比較は第9期計画本文が未受領（No.90）であることを書く"),
]
...
AYAMARI = [
    ...
    ("docx が Markdown を解釈せず ** が本文に出た",
     "強調のつもりで書いた `**` がそのまま印字されていた",
     "python-docx は Markdown を解釈しない",
     "`_runs()` で `**` を分割して太字の run にする。"
     "段落・箇条書き・表のセルの全てを通す"),
    ("在籍と受給の表が12分の1になっていた",
...
P("")
P("**末尾の2件は本書と同時に作った9月業務報告で起きたものです。**"
  "1件は自己点検（「測定済み」の区分があること）が不適合になったことで、"
  "もう1件は再レビューで表を読み直したことで気づきました。"
