...
SHISETSU_ORDER = [
    ...
    "認知症対応型共同生活介護"]
for t in TOWNS:
    rows = block(ALL[TSHEET[t]], "【5　施設・居住系サービスの定員")
    for s in SHISETSU_ORDER:
        v = nums(rows, s)
        exp = T.SHISETSU[t].get(s, 0)
        if v is None:
            ng("A", "施設定員 %s %s" % (t, s), exp, "行がない",
               "サービスの行が見当たらない")
        elif v[0] != exp:
            ng("A", "施設定員 %s %s" % (t, s), exp, v[0],
               "計画作成支援ツールの値と一致しない")
    v = nums(rows, "計")
    exp = sum(T.SHISETSU[t].get(s, 0) for s in SHISETSU_ORDER)
    if v is None or v[0] != exp:
        ng("A", "施設定員の計 %s" % t, exp, v[0] if v else "行がない",
           "町内の定員の合計が一致しない")
...
for s in SHISETSU_ORDER:
    ...
    if v is None:
        ng("A", "比較シート 施設定員 %s" % s, exp, "行がない", "行が見当たらない")
    elif list(v[:3]) != exp or v[3] != sum(exp):
        ng("A", "比較シート 施設定員 %s" % s, exp + [sum(exp)], list(v[:4]),
           "町別の値又は3町計が一致しない")
