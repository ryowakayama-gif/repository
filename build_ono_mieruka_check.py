...
CHOSEI_MIKOMI = 5.963      # 調整交付金見込交付割合（％）第9期の公表値からの逆算
...
def hosei_ninzu():
    ...
    tot = sum(n for _s, _r, n in T.SHOTOKU_R7)
    return sum(r * (n / tot) * pop for _s, r, n in T.SHOTOKU_R7), pop
...
def premium(hyojun_sen, sogo_sen=None, hokatsu_sen=None):
    ...
    chosei_mi = (hyojun_sen + sogo) * CHOSEI_MIKOMI / 100
...
def sheet_hoken(wb, hoken, P):
    ...
    rows = [
        ...
        ("調整交付金見込交付割合（％）", 0, CHOSEI_MIKOMI,
         "**見える化は未入力**。第9期の公表値からの逆算。"
         "見える化には後期高齢者加入割合補正係数（令和9年度0.9558）が入っている"),
...
def build_doc(rows, tot, ku, hoken, P, chiiki, geppo):
    ...
    para("第1号被保険者数も、見える化の第10期3年計10,275人に対し、"
         ...
         "認定者数は見える化が令和9年度849人・令和10年度850人・令和11年度841人としています。")
