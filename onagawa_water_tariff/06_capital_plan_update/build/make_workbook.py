...
ROWS_C=[
 ...
 ('C3','他会計補助金等の控除額','控除しない','控除していない','他会計補助金＋補助金 691,997千円 ／ 推計値 ／ 控除しない',
  '★経営戦略の原価計算表（分解原価集計）は他会計補助金を控除していない。期間原価集計J列の控除項目の●印は'
  '要領上の分類として付されているだけで、最終集計には反映されていない。これに合わせて控除しないのが確定した前提。'
  '繰出基準の整理が変わる場合はここを切り替える。'),
 ('C4','その他営業収益・受取利息・雑収益の控除','控除しない','控除していない','控除する（26,494千円）',
  '期間原価集計では控除項目の●が付いているが、分解原価集計では控除されていない。経営戦略に合わせて控除しない。'),
 ('C5','雑支出の算入','算入しない','算入していない','算入する（36,696千円）',
  '期間原価集計では固定費の●が付いているが、分解原価集計では算入されていない。経営戦略に合わせて算入しない。'),
...
# 確定した前提（2026-09-08 女川町方針）
#   ・繰入金：経営戦略の原価計算表（分解原価集計）は他会計補助金を控除していないため、控除しない
#   ・長期前受金戻入益：経営戦略の実額を確定値として使用
#   ・その他営業収益・受取利息・雑収益、雑支出：経営戦略の原価計算表に合わせて算入・控除しない
REC = dict(RE, hojo='なし', other='控除しない', zatsu='算入しない')
...
CASES = [
 ...
 ('⑤ ④＋動力費・薬品費を補正【確定した前提】', REC),
 ('⑥ ⑤＋他会計補助金272,997を控除（繰出基準が変わる場合）', dict(REC, hojo='他会計補助金')),
 ...
 ('⑫ ⑤＋その他営業収益等を控除し雑支出を算入', dict(REC, other='控除する', zatsu='算入する')),
]
...
head(wv,r,['','箇所','内容','','','','対応']); r+=1
DIFF_C=[
 ('No.3 鷲神高度処理設備の財源構成割合','「財源構成割合(%)」欄に金額（企業債444,000・繰入金444,000）が入力されている。'
  '単純な割合で出せない事業があるため、入力された金額を正として扱う。'
  'ただし様式の財源行の数式が割合前提（整備費×入力値÷100）のままのため、'
  '集計表の「財源：繰入金」が約3.9兆円、「財源：その他」が同額のマイナスになる。',
  '当方で入力金額から直接集計する。様式の数式は今後の混乱を避けるため修正しておく。'
  '女川町への再提出依頼は不要。'),
...
r+=1; band(wr,r,'【C】経営戦略の原価計算表の作り（【料金算定】分解原価集計）',8); r+=1
head(wr,r,['','費目','算定式','4年計（円）','','','内容']); r+=1
SRC_D=[
 ('維持管理費（原浄水）','期間原価集計!O24−O26−O27','255,240,131',
  '原水及び浄水費から動力費・薬品費を除いた額。動力費・薬品費は変動費として別掲。'),
 ('維持管理費（配給水）','期間原価集計!O32−O34−O35','134,698,869','配水及び給水費から同様に控除。'),
 ('維持管理費（その他管理）','期間原価集計!O40','130,727,396','総係費。'),
 ('減価償却費等','固定資産台帳集計結果 T6＋T7＋T8','128,750,077',
  '★取水12,729,056＋浄水16,442,189＋配水99,578,832。損益計算書の減価償却費1,591,381,741ではなく、'
  '固定資産台帳の自己財源相当分のみ。補助財源で取得した資産の償却費は原価に入れない設計。'),
 ('支払利息','固定資産台帳集計結果 T19＋T20＋T21','39,061,345',
  '★H30末の既往債分のみ。期間原価集計O58の新規債利息32,793,807は算入されていない。'),
 ('資産維持費','【料金算定】資産維持費!C12＋C13','41,705,008','対象資産1,042,625,176×1.0%×4年。'),
 ('合計','','734,751,816','料金算定期間はR6〜R9の4年（期間原価集計K4〜N4のラベルはH31・R2・R3・R4のまま）。'),
]
for i,(nm,f_,v,note) in enumerate(SRC_D,1):
    wr.cell(r,1,i).font=F(size=9,color='808080')
    for cn,val in [(2,nm),(3,f_),(4,v),(7,note)]:
        c=wr.cell(r,cn,val); c.font=F(size=9,bold=(cn==2)); c.border=BD
        c.alignment=Alignment(wrap_text=True,vertical='center')
    wr.row_dimensions[r].height=40; r+=1
r+=1
for t in ['★ 控除項目は一切参照されていない。期間原価集計のJ列「控除項目」に●が付いている',
          '　 その他の営業収益・受取利息・他会計補助金・雑収益・長期前受金戻入益は、分解原価集計の算定式に現れない。',
          '　 補助財源分は「減価償却費と支払利息を固定資産台帳ベースに絞る」ことで原価から除く設計になっている。',
          '★ 雑支出（期間原価集計O59・固定費の●あり）も分解原価集計では算入されていない。',
          '★ 期間原価集計の営業外収益の内訳（受取利息＋他会計補助金＋雑収益＋長期前受金戻入益）は、',
          '　 R8で373,468,663円となり、同シートの営業外収益合計432,468,663円と59,000,000円合わない。',
          '　 差額は成行シートの「補助金」で、期間原価集計には行そのものがない。']:
    c=wr.cell(r,2,t); c.font=F(size=9,color=(RED if t.startswith('★') else '595959'))
    wr.merge_cells(start_row=r,start_column=2,end_row=r,end_column=7); r+=1

r+=1; band(wr,r,'【D】建設改良費の施設区分別対比（R8〜R17・千円）',8); r+=1
