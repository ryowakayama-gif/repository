...
CONDUIT = dict(name='北上川導水管（調査票に未計上・復元）', kind='管路',
      cost=[0,670_000,0,0,0,0,0,0,0,0], kokuhi=[0]*10,
      bond=[0,502_500,0,0,0,0,0,0,0,0], rd_s=(40,0), rd_q=(40,3))

PLANS = {'経営戦略の計画': OLD_PLAN, '建設改良費調査票': SURVEY, '調査票＋北上川導水管': SURVEY+[CONDUIT]}
...
ROWS_A=[
 ('A1','建設改良費','建設改良費調査票','経営戦略の計画','調査票 ／ 調査票＋北上川導水管',
...
ROWS_C=[
 ...
 ('C3','他会計補助金等の控除額','成果品の推計値','他会計補助金 272,997千円','他会計補助金＋補助金 691,997千円 ／ 推計値 ／ 控除しない',
  '基準内繰入金は控除、基準外繰入金（赤字補填）は控除しないのが原則。区分が未確定。'),
...
DVS=[
 (f'C{ROW_A["A1"]}','"経営戦略の計画,建設改良費調査票,調査票＋北上川導水管"'),
...
CASES = [
 ...
 ('⑤ ④＋控除項目を経営戦略の実額に', dict(CUR, plan='建設改良費調査票', chouki='実額', hojo='他会計補助金', other='控除する')),
 ('⑥ ⑤＋動力費・薬品費と雑支出を補正', RE),
 ('⑦ ⑥＋機械電気を20年償却', dict(RE, mech=20)),
 ('⑧ ⑥＋北上川導水管を復元', dict(RE, plan='調査票＋北上川導水管')),
 ('⑨ ⑥＋借入利率3.0％', dict(RE, rate=3.0)),
 ('⑩ ⑥で減価償却を固定資産台帳ベースに', dict(RE, dep_meth='固定資産台帳ベース')),
]
...
for t in ['▼ 読み取り方',
  ...
  '・⑦⑧⑨は、確定していない前提（機械電気の耐用年数・北上川導水管・借入利率）が振れた場合の幅を示しています。']:
...
DIFF_A=[
 ...
 ('建設改良費','4,526,000（R8〜R17）','3,515,000（調査票）','中','差 1,011,000。うち670,000は北上川導水管。'),
...
DIFF_C=[
 ...
 ('北上川導水管','経営戦略にはR9に670,000の計上があるが、調査票にない。','中止・繰延・記載漏れのいずれか確認する。'),
