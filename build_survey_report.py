...
（13シート）。各調査内のクロス集計及び公表データとの突合により、
供給構造の面から4調査の結果を接続する。
...
for i, (nm, cont, res, rem) in enumerate([
    ...
    ("11_調査結果の限界と留保", "本報告書の数値を計画本文に用いる際の留保を"
     ...
     "点検事項33件のうち重大5件は取扱いの決定を要する。", "必読"),
    ("12_主要所見と計画本文への反映", "所見と計画素案の反映先を対応させる。",
     "所見20件。うち着手可11件、重大5件の決定待ち9件。", "反映の設計図"),
    ("13_横断クロス集計を行わないことの整理",
     "4調査横断のクロス集計を行わない理由と、"
     "これに代わる接続の方法を記録する。",
...
ws = sheet("03_②居所変更実態調査の結果", "② 居所変更実態調査の結果",
           ...
           "回答した18施設は区域内の施設・住まい26施設のうちの一部であり、"
           "定員は北海道の名簿による（06シート）。",
...
note(ws, r + 1,
     ...
     "区域内の施設・住まい26施設のうち9施設が未回答である（06シート）。"
...
for nm, n, cap, sn, scap, memo in [
    ("介護老人福祉施設（特養）", 3, CAP_TOKUYO, 2, 110,
     ...
     "定員160人は資料依頼No.6③の「定員減少234人→160人」と一致する"),
...
ws = sheet("08_供給構造③_運営法人の集中", "供給構造③　運営法人の集中",
           ...
           "調査結果の偏りが、調査設計ではなく供給構造に由来することを示す。"
           % (N_JIG, N_HOJIN),
...
note(ws, r + 1,
     "注1）本シートは、調査結果の偏りが調査設計に由来するのか"
     "供給構造に由来するのかを分けて示すためのものである。"
     "偏りの存在は調査の欠陥を意味しない。"
     "注2）法人の集中は、"
...
r = header(ws, r, ["区分", "人数", "認定者に対する割合", "", "", "見方"])
for nm, v, pct, memo in [
    ("在宅サービス", 1013, 51.2, "訪問・通所・短期入所等"),
    ("居住系サービス", 144, 7.3, "グループホーム・特定施設"),
    ("施設サービス", 336, 17.0, "特養・老健・地域密着型特養"),
    ("いずれも利用していない", 486, 24.5,
     "認定者1,984人の4分の1。"
     "健康とくらしの調査の「介護・介助が必要だが受けていない7.0％」とは"
     "別の集団であり、合算しない。"
     "内訳（資料依頼No.7）の受領後に要因を分解する"),
]:
...
FIND = [
    ...
    (19, "特定施設の入居者は軽度者が7割である",
     "さわやか東神楽館（定員100人・入居者97人）の要介護度は"
