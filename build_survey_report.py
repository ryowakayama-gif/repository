...
for i, (nm, cont, res, rem) in enumerate([
    ...
    ("14_公表データによる補完と留保の解消",
     ...
     "解消6件・測定済み3件・未解消3件・一部解消1件・"
     "公表待ち1件・決定待ち1件。"
     "訪問介護13事業所181人の全数把握により、"
     "訪問系の代表性の留保が解消した。"
     "令和8年9月11日に年報の要介護度別の明細を収めたことにより、"
...
for no, nm, cant, act, kb in [
    ...
    (2, "①利用者票は課題のある利用者を抽出する設計である",
     "「在宅生活の維持が困難72.7％」を"
     "区域内の在宅利用者全体の割合と読めない。"
     "令和8年9月11日の突合により、偏りの大きさと向きを測定した。"
    ...
    (16, "①の要介護度分布は在宅の認定者の分布と食い違う",
     ...
     % (CHI, CHI_1P, CU_DO_OBS[4] / CU_DO_EXP[4],
        CU_DO_OBS[0] / CU_DO_EXP[0]),
     "留保No.2の測定値である。"
     "代表KPI H12（必要サービス未充足率）も同じ母集団の指標となるため、"
...
for kb, n, cont, when in [
    ("反映済", NST["反映済"],
     ...
     "第2章第4節（高齢者の生活実態）は令和8年9月15日に"
     "健康とくらしの調査の結果を入れ、4段落・表1から30段落・表8となった。"
     "第6章第2節・第4節は令和8年9月16日に"
     "サービス見込量の算定 第1次概算の値へ差し替えた。", "反映済"),
    ("着手可", NST["着手可"],
     "点検事項の取扱いの決定を要しない。"
     "計画素案の第1章第7節・第2章第3節・第2章第6節・"
     "第3章第5節・第5章・第6章第5節へ反映する。", "R8.10"),
    ("決定待ち", NST["決定待ち"],
...
note(ws, r + 1,
     "注1）所見%d件のうち%d件は反映済み、%d件は"
     "点検事項の取扱いの決定を要せず着手できる。"
     "注2）反映先は計画素案の章節による。"
     "実際の反映は別管理表で管理し、計画本文には確認事項の注記を書かない。"
     "注3）本シートの所見は受託者によるものであり、"
     "計画本文への反映は発注者のご意向を確認したうえで行う。"
     "注4）「反映済」は、計画素案（%s）の本文と表を実際に読んで"
     "判定している。固定値では書いていない。"
...
for a in [
    ("7", "①利用者票は課題のある利用者を抽出する設計である"
     ...
     "区域内の在宅利用者全体ではない。"
     "ただし令和8年9月11日に年報（令和7年度）の要介護度別の明細を"
     "収めたことにより、偏りの大きさと向きを測定できるようになった。",
...
note(ws, r + 1,
     ...
     "令和8年9月11日に偏りの大きさと向きを測定できるようになったが、"
     "留保そのものは解消していない。"
