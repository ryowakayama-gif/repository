...
for i, (nm, cont, res, rem) in enumerate([
    ...
    ("11_調査結果の限界と留保", "本報告書の数値を計画本文に用いる際の留保を"
     ...
     "必読"),
    ("12_主要所見と計画本文への反映", "所見と計画素案の反映先を対応させ、"
     "反映済みかどうかを示す。",
     "所見20件。反映済み・着手可・決定待ちに区分する。",
     "反映の設計図"),
    ("13_横断クロス集計を行わないことの整理",
     "4調査横断のクロス集計を行わない理由と、"
     "これに代わる接続の方法を記録する。",
...
for nm, v, use in [
    ...
    ("勤続1年未満の割合", "19.9％",
     "定着の補足指標。代表KPI H14は「採用率と離職率の差」であり、"
     "本値はH14そのものではない"),
...
# 本報告書は発注者へ送付するものであり、当方の内部の仕組み・作業経過を
# 書かない（令和8年9月24日ご指示。CLAUDE.md §1）。
_NAIBU_GO = ["固定値", "実物から", "章節ごとに", "判定している",
             "読んで判定", "スクリプト", "runpy", ".py",
             "再実行", "書き写して"]
_naibu_hit = [w for w in _NAIBU_GO
              if any(w in c for c in _ALL if "禁止" not in c)]
chk(12, "受託者の内部の仕組み・作業経過の語が残っていないこと",
