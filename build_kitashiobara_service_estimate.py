...
- 第8期で新設された別表第五は、全部過疎市町村でなく、かつサービス利用者割合が
  ...
  本村は全部過疎市町村に該当すると考えられ適用対象外だが、過疎区分の確認が必要
  （12_別表第五判定シート）。
...
def sheet_overview(wb):
    ...
    rows = [
        ...
        ("12_別表第五判定", "地域差是正規定の適用判定", "算定方法の前提", "村確認待ち",
         "本村の過疎区分が確定するまで算定方法は確定しない"),
    ]
...
def sheet_beppyo5(wb):
    ws = add_sheet(
        ...
        "本村が該当するかを確定するまで、見込量の算定方法は確定しない。",
        [28, 20, 20, 52])
    ...
    # ⑴ 過疎区分
    write_row(ws, r, ["⑴ 本村の過疎区分", "全部過疎（見込）", None,
                      "総務省が公表する過疎地域市町村一覧で確認する。"
                      "北塩原村は過疎地域であり、市町村合併を経ていないため全部過疎と考えられる。"],
              aligns=["left", "center", "center", "left"])
    ws.cell(row=r, column=3).value = f'=IF(B{r}="全部過疎（見込）","要件⑴を満たさない→別表第五 適用外","要確認")'
    style_data_cell(ws.cell(row=r, column=3), align="center")
    ws.cell(row=r, column=2).fill = PatternFill("solid", fgColor=COLORS["input"])
    row_kaso = r
    r += 1

    # ⑵ サービス別の利用者割合
    write_row(ws, r, ["⑵ サービス利用者割合", "村資料・国公表データ待ち", "―",
    ...
    write_row(ws, r, ["結論", None, None,
                      "⑴かつ⑵の連言であるため、⑴を満たさなければ別表第五 二の項の算定方法は適用されない。"],
              aligns=["left", "center", "center", "left"])
    ...
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=4)
    style_note(ws.cell(row=r, column=1),
               "※ 仮に本村が全部過疎に該当しない場合でも、第三の二の2(一)⑵により、"
               "地域の具体的なニーズ調査を踏まえて別の算定方法を採ることができる。"
               "その場合は、個別積上げ方式を採る必要性及び根拠を計画本文に明記することが要件となる。")
    ws.row_dimensions[r].height = 34
    r += 1
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=4)
    style_note(ws.cell(row=r, column=1),
