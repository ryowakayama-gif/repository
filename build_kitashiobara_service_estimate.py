...
# 別表第五（地域差是正）の対象となる特定障害福祉サービス・特定障害児通所支援
# 「障害福祉計画策定に係る実態調査及びPDCAサイクルに関するマニュアル」（令和8年8月）P.22
BEPPYO5_SERVICES = ["生活介護", "就労継続支援A型", "就労継続支援B型",
...
def sheet_beppyo5(wb):
    ...
    style_note(ws.cell(row=r, column=1),
               ...
               "「障害福祉計画策定に係る実態調査及びPDCAサイクルに関するマニュアル」（令和8年8月）P.22 は、"
               "対象を " + "、".join(BEPPYO5_SERVICES) + " の6サービスと明示している。"
               "本村はこの6サービスすべてに実績がある。")
    ws.row_dimensions[r].height = 46
    r += 1
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=4)
    style_note(ws.cell(row=r, column=1),
               "※ 上記マニュアル P.22 手順Ⅰは「要件1と要件2の両方に該当しない場合は、"
               "18ページの内容を参考として、サービスに必要な見込量を算出してください」としており、"
               "全部過疎市町村である本村が地域差是正の算定方法の対象外であることを確認できる。"
               "同 P.21 は、実績値が小さい場合は「実績値の変化により推計値の値が変化しやすい」、"
               "実績値が極端に増減している場合は「幾何平均による算出では2年後・3年後の推計値が"
               "過大（過少）に推計されてしまう可能性がある」と留意点を示しており、"
               "利用者が各サービス数人規模の本村で個別積上げ方式を採る根拠になる。"
