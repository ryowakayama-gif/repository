...
kpi_rows.append(("―", "介護・介助が必要だが受けていない者の割合（新規提案）", "計画素案では設定なし",
                 "健康とくらしの調査【問1】2）",
...
note(ws, r + 1,
     ...
     "注2）所見2〜12は計画素案への反映を要する。"
     "反映は図表レイアウトの確認結果と併せて次の改訂で行う。"
...
REVIEW = [
    ...
    (3, "重度者の在宅化が進む一方（要介護5の在宅・居住系割合31.3％→54.6％）、"
     ...
     "困りごとの分析を追加した。", "10・15"),
    (5, "3町の高齢者福祉計画のうち、事業の数値目標を設定しているのは東川町のみ",
     "資料2 3町との役割分担・共通指標",
    ...
    ("―", "施策2-3 認知症施策推進大綱を踏まえた施策の推進（指標不足）",
     ...
     "計画素案では認知症の指標が見える化のデータ登録がないため空白であった。"
