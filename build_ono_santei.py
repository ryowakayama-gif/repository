...
def sheet_hokenryo(wb, hyojun_rows, chiiki):
    ...
    for name, rate, num in SHOTOKU_R7:
        ws.append([name, TARGET[name], rate, round(num / act_tot * pop3), None])
    s1 = ws.max_row
    ...
    ws.append(["※ 補正後人数＝保険料率×被保険者数。この合計÷被保険者数の合計が"
               "「所得段階別加入割合補正係数」で、調整交付金の算定にも用います。"])
    ws.append(["※ **第9期の算定における補正係数は0.968でしたが、令和7年度実績では0.952です。**"
               "低所得段階の割合が上がっており、同じ賦課総額でも基準額が上がる方向に働きます。"
               "第9期の算定に用いた所得段階別人数の年度を町に確認してください。"])
    return ws
