...
def sheet_village(wb, rows):
    ws = add_sheet(
        ...
        "ReMHRAD「在・退院者の状況」タブの指標「在院患者数_患者の住所ベース」（出典：630調査）による。"
        "入院期間による絞り込みはなく、患者住所地が北塩原村である全在院患者数である。"
        "別表第四のCが求める1年以上入院患者数とは範囲が異なることに注意。",
        [14, 20, 26, 40, 40])
    ...
    for t in [
        ...
        "※【確定】ReMHRAD「ReMHRADについて」のデータソース一覧により、"
        "本表の指標は「在院患者数_患者の住所ベース」（出典：630調査）であり、"
        "入院期間による絞り込みはない。すなわち全在院患者数である。",
        "※現行計画は令和5年度の1年以上長期入院者を5人と記載しており、"
        "本表の令和5年（2023年）10人が全在院患者であることと矛盾しない（10人のうち5人が1年以上）。",
        "※別表第四のCは1年以上入院患者数である。本村の基盤整備量を定める際は、"
        "本表の全在院患者数ではなく1年以上の人数を用いること。"
        "1年以上の人数と年齢区分（65歳未満／65歳以上／75歳以上／40歳以上の認知症）の内訳は、"
        "村・会津保健所への照会が必要。",
    ]:
