...
r = header(ws, 4, ["区分", "施設数", "定員", "入所者", "入所率", "空き",
                   "備考"])
for g in ["特養・地域密着型特養", "介護老人保健施設", "グループホーム",
    ...
    if g == "特養・地域密着型特養":
        res = d["入所"] + 58
        memo = "特別養護老人ホーム美瑛慈光園の入所者数が未記入のため、" \
               "要介護度別の内訳58人を加えた"
    r = body(ws, r, [g, d["施設数"], d["定員"], res,
                     "%.1f％" % (res / d["定員"] * 100) if d["定員"] else "―",
                     d["定員"] - res, memo], height=32, align=RIGHT)
tot_cap = sum(d["定員"] for d in CAP.values())
tot_res = sum(d["入所"] for d in CAP.values()) + 58
r = body(ws, r, ["計", sum(d["施設数"] for d in CAP.values()), tot_cap,
                 tot_res, "%.1f％" % (tot_res / tot_cap * 100),
                 tot_cap - tot_res, ""], {1: GRAY}, height=20, align=RIGHT,
         bold=True)
...
r = header(ws, r, ["区分", "令和7年度\n（実績）", "令和11年度\n（見込み）",
                   "定員", "定員に対する割合", "判定", "見方"])
n29 = nin("2029")
for kub, cap in [("施設", CAP["介護老人保健施設"]["定員"]
                  + CAP["特養・地域密着型特養"]["定員"]),
                 ("居住系", CAP["グループホーム"]["定員"]
                  + CAP["特定施設"]["定員"])]:
    act = ACT[kub]
...
note(ws, r + 1,
     ...
     "注2）居住系サービスの利用者数（令和11年度145人）は"
     "区域内の定員139人（グループホーム81人＋特定施設58人）を上回る。"
     "これは令和7年度の実績（144人）でも同じであり、"
     "区域外の居住系施設を利用している分が含まれているためと考えられる。"
     "介護サービス自給率（δ）で施設・居住系の区域外利用が"
     "東川町23.2％・美瑛町24.7％・東神楽町34.5％であることと整合する。"
     "定員による制約としてではなく、区域外利用の構造として読む。"
     "施設サービスは定員392人に対し339人であり余裕がある。"
     "注3）住宅型有料老人ホーム・サービス付高齢者向け住宅は"
     "介護保険の指定サービスではないため、"
     "見込量（居住系サービス）には含まれない。"
     "定員121人・入居者115人は住まいの供給量として第2章第3節に掲載する。"
