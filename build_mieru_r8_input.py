...
2 人数の欄は整数で扱われる（確認事項No.135。令和8年9月11日に解決）。
  利用回（日）数の欄は小数第1位まで受け付ける。
  定期巡回・随時対応型訪問介護看護は月平均0.583人、
  認知症対応型通所介護は月平均1.000人であり、
  各欄を単純に四捨五入すると全欄が0になって
  令和9年度から令和11年度も0のまま残る。
  本表の整数版は群（要支援・要介護）ごとに合計を保つため0にはならない。
...
def to_dec1(vals):
    """小数第1位に丸める。群ごとに合計（小数第1位）を保つ（最大剰余法）。
    ...
    for g in (SHIEN_I, KAIGO_I):
        ...
        tgt = int(Decimal(str(s * 10)).quantize(Decimal("1"), ROUND_HALF_UP))
        base = {i: int(vals[i] * 10) for i in g}
        need = tgt - sum(base.values())
        order = sorted(g, key=lambda i: -(vals[i] * 10 - int(vals[i] * 10)))
        for i in g:
            out[i] = base[i]
        k = 0
        while need > 0:
            ...
            k += 1
        while need < 0:
            for i in sorted(g, key=lambda i: (vals[i] * 10 - int(vals[i] * 10))):
                if out[i] > 0 and need < 0:
                    out[i] -= 1
                    need += 1
    return [x / 10.0 for x in out]
...
for i, (a, b) in enumerate([
    ("画面の値と年報の月平均が食い違う",
     ...
     "本表は年報の値を用いる（05シート）"),
    ("人数の欄は整数で扱われるため丸め方で小口が消える",
     "定期巡回・随時対応型訪問介護看護は月平均0.583人、"
     "認知症対応型通所介護は月平均1.000人である。"
     "各欄を単純に四捨五入すると全欄が0になり、"
     "令和9年度から令和11年度も0のまま残る。"
     "本表の整数版は群（要支援・要介護）ごとに合計を保つため"
     "0にはならない（05シート1）"),
...
r += 1
r = lead(ws, r, "3　入力に併せて設定する項目", span=5)
r = header(ws, r, ["", "設定項目", "当方の案", "理由", ""])
for i, (a, b, c) in enumerate([
    ("利用者数（人数）の欄",
     "整数版（02・03シートの下段）を入力する",
     "人数の欄は整数で扱われる。"
     "小数を入れても令和9年度から令和11年度の推計値は整数で出る。"
     "整数版は最大剰余法により要支援・要介護の群ごとの合計を保つため、"
     "定期巡回・随時対応型訪問介護看護（月0.583人）は"
     "要介護5に1人が立ち、0にはならない"),
    ("利用回（日）数の欄",
     "小数第1位版（04シートの★の行）を入力する",
     "利用回（日）数の欄は小数第1位まで受け付ける"),
    ("実績見込み値を編集した理由の記入欄",
     "「令和8年度の実績が4か月分であるため」",
     "設定内容を記入する欄が画面にある。"
     "記入した内容は推計結果の出力に「施策反映の内容」として載る。"
     "後日の説明のために記入しておく"),
    ("認定率の伸び・利用率の伸びの選択",
     "「当該市町村における令和6年度→令和7年度の伸び」を選ぶ",
     "令和8年度を含まない唯一の選択肢である。"
     "令和8年度は4か月であり、在宅は夏季に偏る"
     "（当方の検証では在宅は冬季が夏季より2.48％低い）"),
    ("1人1月あたり給付費の実績値の年度",
     "［要協議］令和7年度を選ぶ案",
     "令和7年度か令和8年度かを選ぶ項目がある。"
     "当方の検証では単価は改定の有無に関係なく年率1.75％で伸びており、"
     "令和7年度で固定することは月額で＋185〜＋374円に当たる。"
     "施設・居住系と在宅で別に選べるため、"
     "そろえるかどうかを含めご判断いただきたい（確認事項No.137）"),
], start=1):
    r = body(ws, r, [i, a, b, c, ""],
             fills={3: (IN_Y if "要協議" in b else OK_G)}, height=62)
r = note(ws, r + 1,
         "注）本節の①から⑤までは、弊社の別案件において"
         "実際に小数を入力して出力された推計結果により確かめたものです。"
         "その保険者の数値は用いていません。", span=5)
...
r = note(ws, r + 1,
         "注1）人数の欄は整数で扱われるため、"
         "下段の「（整数）」が入力値です（00シート3参照）。"
         "上段の★の行は年報の値を12で除したそのままの値で、"
         "整数版はこれを群（要支援・要介護）ごとに"
         "合計を保って丸めたものです。\n"
         "注2）施設・居住系の計は{:.3f}人です。"
...
r += 1
r = lead(ws, r, "2　入力値（上段が実数、下段の整数が入力値）", span=10)
...
r = note(ws, r + 1,
         "注2）人数の欄は整数で扱われるため、"
         "下段の「（整数）」が入力値です（00シート3参照）。\n"
         "注3）整数に丸めると計が0又は1になるサービスが{}件あります"
         "（{}）。"
         "群ごとに合計を保つ丸め方によるため、"
         "定期巡回・随時対応型訪問介護看護（月0.583人）は"
         "要介護5に1人が立ち、0にはなりません。"
         "各欄を単純に四捨五入すると全欄が0になり、"
         "令和9年度から令和11年度も0のまま残ります。\n"
...
for nm, tan, v in KAISU:
    dv = to_dec1(list(v))
    r = body(ws, r, ["（実数）", nm, tan, r3(sum(v))] + [r3(x) for x in v],
             numfmt={k: "0.000" for k in range(4, 12)}, height=20)
    r = body(ws, r, ["★", "", "", r3(sum(dv), 1)] + [r3(x, 1) for x in dv],
             fills={k: OK_G for k in range(4, 12)},
             numfmt={k: "0.0" for k in range(4, 12)}, height=20)
    r = body(ws, r, ["（2区分）", "", "",
                     r3(sum(dv), 1),
                     r3(sum(dv[i] for i in SHIEN_I), 1), "", "", "",
                     r3(sum(dv[i] for i in KAIGO_I), 1), "", ""],
             fills={5: MID_B, 9: MID_B},
             numfmt={k: "0.0" for k in (4, 5, 9)}, height=18)

r = note(ws, r + 1,
         "注1）★の行（小数第1位）が入力値です。"
         "利用回（日）数の欄は小数第1位まで受け付けます"
         "（05シート1参照）。"
         "丸めは群（要支援・要介護）ごとに合計を保つ最大剰余法によります。"
         "上段の「（実数）」は年報の値を12で除したそのままの値です。"
...
r = 4
r = lead(ws, r, "1　入力欄が受け付ける桁（確認事項No.135の回答）", span=6)
r = header(ws, r, ["", "欄", "実績（令和6・7年度）",
                   "実績見込み（令和8年度）", "推計（令和9〜11年度）",
                   "本表で用意するもの"])
for i, (a, b, c, d, e) in enumerate([
    ("利用者数（人数）", "小数（÷12の値のまま）", "すべて整数", "すべて整数",
     "整数版（02・03シートの下段）"),
    ("利用回（日）数", "小数", "小数第1位まで", "小数第1位まで",
     "小数第1位版（04シートの★の行）"),
], start=1):
    r = body(ws, r, [i, a, b, c, d, e],
             fills={4: MID_B, 5: MID_B, 6: OK_G}, height=32)
r = note(ws, r + 1,
         "注1）弊社の別案件において、令和8年度の実績見込み値を編集し"
         "小数を入力したうえで出力された推計結果により確かめたものです。"
         "同じ表の中で令和6・7年度の人数は小数10桁まで表示されるのに対し、"
         "編集した令和8年度の人数は全45サービスが整数、"
         "令和9年度から令和11年度の人数も全135件が整数でした。"
         "利用回（日）数は令和8年度・令和9年度以降とも小数第1位まで"
         "出ています。その保険者の数値は用いていません。\n"
         "注2）したがって、人数については"
         "「小数で入力できるか」ではなく"
         "「整数に丸めたときに小口が消えないか」が論点になります。"
         "本表の整数版は群（要支援・要介護）ごとに合計を保つため、"
         "定期巡回・随時対応型訪問介護看護は要介護5に1人が立ちます。",
         span=6)

r += 1
r = lead(ws, r, "2　施設・居住系　画面の令和7年度・年報÷12・総括表÷12",
         span=6)
...
_kesu = [nm for nm, _s, _x in ZAITAKU
         if ZAITAKU_KEI[nm] > 0 and sum(to_int(ZAITAKU_DO[nm])) == 0]
chk(14, "整数版で実績のあるサービスが消えないこと",
    "在宅{}サービス".format(len(ZAITAKU)),
    "消えるものはない（定期巡回は月0.583人で要介護5に1人が立つ）"
    if not _kesu else "消える " + "・".join(_kesu),
    not _kesu)

_d1 = [nm for nm, tan, v in KAISU
       if any(abs(sum(to_dec1(list(v))[i] for i in g)
                  - sum(v[i] for i in g)) > 0.05 + 1e-9
              for g in (SHIEN_I, KAIGO_I))]
chk(15, "利用回（日）数の小数第1位版が群（要支援・要介護）ごとに"
        "実数と0.05を超えて離れないこと",
    "{}サービス×2群".format(len(KAISU)),
    "全件0.05以内" if not _d1 else "超過 " + "・".join(_d1), not _d1)
...
KAKUNIN = [
    ("No.134", "年報（令和7年度）の様式別データのご提供　→　取下げ",
     ...
     "（解消済み）",
     "―", "―"),
    ("No.135", "入力欄が小数を受け付けるか　→　解決",
     "令和8年9月11日、弊社の別案件において実際に小数を入力して"
     "出力された推計結果により次を確かめた。"
     "①利用者数（人数）の欄は整数で扱われる"
     "（令和6・7年度の実績は小数10桁まで表示されるのに対し、"
     "編集した令和8年度の人数は全45サービスが整数、"
     "令和9年度から令和11年度の推計値も全135件が整数）。"
     "②利用回（日）数の欄は小数第1位まで受け付ける。"
     "本表は人数を整数版（群ごとに合計を保つ最大剰余法）、"
     "利用回（日）数を小数第1位版として用意した。"
     "定期巡回・随時対応型訪問介護看護（月0.583人）は"
     "要介護5に1人が立ち、0にはならない。",
     "（解消済み）",
     "―", "―"),
    ("No.136", "画面の令和7年度と年報の月平均の食い違い",
     ...
     "将来推計 第2段階\n計画素案 第6章第2節",
     "発注者", "R8.10"),
    ("No.137", "1人1月あたり給付費の実績値をどの年度にするか",
     "「推計方法の設定」には、1人1月あたり給付費の実績値を"
     "令和7年度とするか令和8年度とするかを選ぶ項目があり、"
     "施設・居住系と在宅で別に選べる。"
     "①当方は令和7年度（12か月の完結年度）を選ぶ案としたい。"
     "②ただし当方の検証では、単価は介護報酬改定の有無に関係なく"
     "年率1.75％で伸びており（構成を固定した連鎖ラスパイレス指数で"
     "7年＋12.93％）、令和7年度で固定することは月額で"
     "＋185〜＋374円に当たる。"
     "③令和9年度は介護報酬改定の年である（3年周期。第10期の初年度）。"
     "改定率1％あたり月額＋61円である。"
     "④施設・居住系と在宅で年度をそろえるかどうかも"
     "併せてご判断いただきたい。",
     "将来推計 第2段階\nサービス見込量 第1次概算",
     "発注者", "R8.9"),
]
...
r = note(ws, r + 1,
         "注1）本表に係る確認事項は{}件（No.134〜No.137）で、"
         "うちNo.134は取下げ、No.135は解決しました。"
         "ご判断・ご確認をお願いするのはNo.136・No.137の2件です。\n"
...
print("確認事項：{}件（No.134取下げ・No.135解決・No.136/137が未回答）"
      .format(len(KAKUNIN)))
