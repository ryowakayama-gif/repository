...
当方が先にお示ししたのは②③の2画面だけであった。
  ...
  ① 認定者数の実績値          編集項目の有無を要確認（確認事項No.138）
  ② 施設・居住系サービス利用者数
  ③ 在宅サービス利用者数
  ④ 在宅サービス利用回（日）数

②③④はそのまま入力できる。
①について、当方は令和8年9月11日に「差し替えは4画面ある」とお伝えしたが、
受領した画面の写しで「実績及び推計方法の設定」に列挙されている編集項目は
（3）1）（4）1）（4）2）の3件であり、
認定者数の実績見込み値を編集する項目は写っていない。
画面の認定率は令和7年度・令和8年度とも20.84％で同じであり、
令和8年度は既に令和7年度の率で置かれている可能性がある。
編集項目がない場合は、認定率の伸びに
「令和6年度→令和7年度の伸び」を選ぶことで令和8年度の影響を避ける。
...
r = 4
r = lead(ws, r, "1　差し替える項目と出所", span=5)
...
r = note(ws, r + 1,
         "注1）②③④はそのまま入力していただけます。"
         "①については、当方が先に「4画面目」としてお示ししましたが、"
         "編集する項目が画面にあるかを確かめられていません"
         "（確認事項No.138）。"
         "受領した画面の写しで「実績及び推計方法の設定」に"
         "列挙されている編集項目は（3）1）施設・居住系の利用者数、"
         "（4）1）在宅の利用者数、（4）2）在宅の利用回（日）数の3件で、"
         "認定者数の実績見込み値を編集する項目は写っていません。"
         "また画面の認定率は令和7年度・令和8年度とも20.84％で同じであり、"
         "令和8年度は既に令和7年度の率で置かれている可能性があります。"
         "編集する項目がない場合は、"
         "認定率の伸びに「令和6年度→令和7年度の伸び」を選ぶことで"
         "令和8年度の影響を避ける形になります（本節3の④）。\n"
...
KAKUNIN = [
    ...
    ("No.136", "画面の令和7年度と年報の月平均の食い違い",
     ...
     "「令和6年度年報及び令和7年度、8年度の最新の月報」と記されている。"
     "画面の令和7年度が年報ではなく月報によるものであれば"
     "食い違いの説明になる。当方は月報（令和7年度）を保有していないため"
     "確かめられていない。"
    ...
    ("No.137", "1人1月あたり給付費の実績値をどの年度にするか",
     ...
     "将来推計 第2段階\nサービス見込量 第1次概算",
     "発注者", "R8.9"),
    ("No.138", "認定者数の実績見込み値を編集する項目が画面にあるか",
     ...
     "②画面の認定率は令和7年度・令和8年度とも20.84％で同じであり、"
     "令和8年度は既に令和7年度の率で置かれている可能性がある。"
     "③編集する項目がある場合は01シートの実数（1,984人）を入力する。"
...
r = note(ws, r + 1,
         "注1）本表に係る確認事項は{}件（No.134〜No.138）で、"
         "うちNo.134は取下げ、No.135は解決しました。"
         "ご判断・ご確認をお願いするのはNo.136・No.137・No.138の3件です。"
         "このうちNo.138（認定者数の編集項目の有無）は"
         "入力を始める前にご確認いただく必要があります。\n"
...
print("確認事項：{}件（No.134取下げ・No.135解決・No.136/137/138が未回答）"
      .format(len(KAKUNIN)))
