...
# 過去の時点の素案の規模（段落・表・図）。
# **過去の時点を記す資料（月次の業務進捗報告書・議事録）は、
#   draft_label() ではなくこちらを用いる。**
# draft_label() は現時点の実物を数えるため、素案を改訂するたびに
# 過去の報告の数値まで書き換わってしまう。
# （令和8年9月17日の点検で、業務進捗報告書 令和8年8月分の
#   「協議用素案（令和8年8月時点）」の規模が、再出力のたびに
#   667段落114表34図→806段落130表36図→843段落137表36図と
#   3回書き換わっていたことが分かった。）
# 値は、当該日付までの最後のコミットの素案の実物を
# draft_size() と同じ方法で数えたものである。
DRAFT_KIROKU = {
    # ee2ccac（令和8年8月31日）の素案を数えたもの
    "令和8年8月31日": (741, 124, 39),
}


def draft_label_at(kijunbi):
    """過去の時点の素案の規模を「741段落124表39図」の形で返す。"""
    p, t, z = DRAFT_KIROKU[kijunbi]
    return "%d段落%d表%d図" % (p, t, z)
