...
# 令和8年9月25日に受領し、解消したもの／解消しなかったもの。
KAISHO = [
    ("**解消した**", "小野町の交付金の得点",
     "**W126〜W137を受領した。総合233.0点（全国422.4・福島県354.1）。**"
     "0点の項目16件と全国を上回る項目11件を特定した"
     "（08_協議会資料／小野町_交付金_小野町の得点分析）"),
    ("**解消した**", "令和8年度が何か月分か",
     "**介護保険事業状況報告の設定で、令和8年度は5月のみチェック。**"
     "令和8年5月の1か月であることが設定画面で確定した"),
    ("**解消した**", "推計方法の設定",
     "**基本推計。認定率の伸びは令和6年度→令和7年度、"
     "利用率と1人1月あたり利用回（日）数の伸びはいずれも0。**"
     "当方の自然体推計の前提が裏づけられた"),
    ("**解消した**", "施策反映が入っているか",
     "**入っていない。**3画面とも「自然体推計に全て戻す」がグレーアウト"),
    ("**解消しなかった**", "第9期策定時の設定",
     "**第9期の推計パターンはシステムに残っていない。**"
     "画面は推計名が空欄で保険料も「―」。"
     "第9期の乖離の原因は当方の推定にとどまる"),
    ("**解消しなかった**", "認知症関連の研修の受講状況（J1〜J15）",
     "**受領したが、小野町の値はすべて「−」であった。**"
     "都道府県が実施する研修であり、市町村別の値がない。"
     "**当方の推奨が外れていた。**町の事業実績による"),
    ("**解消しなかった**", "生活支援コーディネーター・協議体（F26・F27）",
     "**受領したが小野町の値は「−」。**令和2年が最新であり、"
     "市町村別の値がない"),
    ("**一部解消**", "地域包括支援センターの体制（F15〜F25）",
     "**令和2〜6年度の時系列で受領した。**"
     "センター設置数は委託型が65歳以上人口1万対2.87（令和6年度）で、"
     "実数1か所。3職種の配置は保健師0・保健師に準ずる者2.87ほか"),
    ("**新たに分かった**", "「推計パターン毎の乖離状況」ボタン",
     "**推計方法の設定画面にこのボタンがある。**"
     "厚生労働省の保険者別配布シートに相当する内容が"
     "システム内で見られる可能性がある。表示内容を町に照会中"),
]
...
def build_docx(M, req, got, A):
    ...
    head(doc, "0　令和8年9月25日の受領による解消状況", 1)
    body(doc,
         "**本書は令和8年9月25日に作成したものですが、"
         "同日中に町から見える化システムの出力58件と画面キャプチャ8枚を"
         "受領したため、解消状況を先に記します。**"
         "以下の第1章以降は、受領前の整理です。")
    table(doc, ["状況", "事項", "内容"],
          [(a, b, c) for a, b, c in KAISHO])

    head(doc, "1　（受領前の整理）結論", 1)
    table(doc, ["区分", "内容"], [
        ("**A 見える化から取れる**",
    ...
    head(doc, "2　A　見える化システムから取れるもの", 1)
    body(doc,
    ...
    head(doc, "3　とくに優先していただきたいもの ― W126〜W137", 1)
    body(doc,
         "**これは小野町の交付金の得点そのものです。**"
    ...
    head(doc, "4　B　見える化システムでは取れないもの", 1)
    table(doc, ["未確認事項", "見える化に無い理由", "解消の方法"],
          [(nm, why, how) for nm, why, how in BLOCK_B])

    head(doc, "5　C　見える化システム上で自前で確かめるもの", 1)
    head(doc, "5-1　推計手法（基本推計／包括推計）はどちらが当たるか", 2)
    body(doc,
