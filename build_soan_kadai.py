...
_s24r = next(((x[2], x[3]) for x in SETSU_RYO
              if x[1] == "第4節 高齢者の生活実態"), (0, 0))
r = note(ws, r + 1,
         ...
         "{}節です。\n"
         "注5）第2章第4節（高齢者の生活実態）は令和8年9月15日に"
         "健康とくらしの調査の結果を反映し、{}段落・表{}になりました。"
         "記述が薄い節には当たらなくなっています。"
         .format(len(USUI), len(_toriaezu), *_s24r),
         span=8)
...
KITEI = {
    ...
    128: "見える化システムの令和8年度の実績見込み値は編集しないまま概算する。"
         "当方の算定は年報（令和7年度）によるため見込量は動かず、"
         "システムの出力との差として残る",
    129: "令和8年度（4か月）を含まない④（令和6年度→令和7年度）の伸びを"
         "選んだものとして概算する。当方の算定（令和7年度基準）と置き方がそろう",
    126: "基本推計に近いP3で概算する",
    ...
    145: "サービス見込量・給付費・保険料は年報（保険者単位）の値で概算し、"
         "町別データシートは計画作成支援ツールの値のまま示す。"
         "3町を足しても年報の値にならないことを注記する",
    151: "認知症対応型共同生活介護及び介護老人福祉施設を"
         "対計画比の補正の対象から外したものとして概算する。"
         "定員の縮小を趨勢として二重に織り込まないため（安全側）",
    147: "令和8年度を含まない④（令和6年度→令和7年度）を採ったものとして"
         "概算する。当方の算定（令和7年度基準）と置き方がそろう",
    148: "受領済みの画面の写しの範囲で概算し、"
         "確認できていない項目はその旨を明記する",
    146: "要介護度別の伸びによる算定（第1次概算）で概算し、"
         "一律の伸びによる従前の値を併記する。差は月額＋8円である",
...
MIRYO = [
    ...
    ("④の分析結果の計画本文への反映",
     ...
     "社会参加や暮らし向きとの関連分析。"
     "令和8年9月15日に第2章第4節へ反映した（8表・30段落）。"
     "地区の定義は［要確認］のまま残している",
     "第2章第4節（反映済み）・第5章 基本目標1（未）",
     "No.9（地区の定義の部分）", "一部完了"),
...
for i, (a, b, c_, d, e) in enumerate(MIRYO, start=1):
    r = body(ws, r, [i, a, b, c_, d, e],
             fills={6: {"実施中": OK_G, "一部完了": OK_G, "確認待ち": IN_Y,
                        "未着手": GRAY}.get(e)}, height=52)

_jisshi = [x for x in MIRYO if x[4] in ("実施中", "一部完了")]
r = note(ws, r + 1,
...
DEKIRU = [
    ("④の分析結果の第2章第4節への反映",
     "令和8年9月15日に反映した。調査の対象者の範囲・主要指標の町別・"
     "年齢調整と町間の差の検定・同規模保険者40との比較・"
     "社会参加との関連・暮らし向きとの関連・生活実態の主な値・"
     "地区別の8項目で、8表を掲げた。"
     "集計値は `data_survey_jages.py` に収め、個票なしで再現できる。"
     "地区の定義は［要確認］のまま残している",
     "第2章第4節", "完了",
     "アンケート分析 未了No.4"),
    ("中長期推計（令和22年度まで）の算定",
...
for i, (a, b, c_, d, e) in enumerate(DEKIRU, start=1):
    r = body(ws, r, [i, a, b, c_, d, e],
             fills={5: (OK_G if ("着手できる" in d or d == "完了")
                        else IN_Y)}, height=56)
...
_s24 = next(((x[2], x[3]) for x in SETSU_RYO
             if x[1] == "第4節 高齢者の生活実態"), (0, 0))
chk(12, "第2章第4節に健康とくらしの調査の結果が入っていること"
        "（令和8年9月15日に反映）",
    "同節の段落数と表数",
    "{}段落・表{}。記述が薄い節には当たらない".format(*_s24),
    not any(x[1] == "第4節 高齢者の生活実態" for x in USUI))
