...
ws = sheet("00_送付資料一覧", "送付資料一覧（令和8年8月）",
           ...
           "受託者の判断・推奨・試算・留保が含まれます。確定値ではありません。",
           [5, 42, 52, 10, 20])
...
for gname, files in GROUP:
    ...
    for fn in files:
        ...
        r = body(ws, r, [i, fn, why, scale_of(p),
                         "そのまま" if kb == SOFU else "お諮りする内容を含む"],
                 {5: OK_G if kb == SOFU else IN_Y}, height=34,
                 align={1: "center", 4: "center", 5: "center"})

r += 1
r = body(ws, r, ["計", i, "うち そのまま %d件／お諮りする内容を含む %d件"
                 % (cnt[SOFU], cnt[JOKEN]), "", ""],
...
for a in [
    (1, "人口推計の基礎に何を用いるか"
     ...
     "認定者数・サービス見込量・給付費・保険料のすべてが確定しません。"
     "第6章の算定が着手できません。",
