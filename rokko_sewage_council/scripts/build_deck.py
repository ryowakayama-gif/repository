...
put(SP("nochange"), {
    ...
    "rec9": ("▶ 改定しない場合、令和8〜16年度の9年間で両事業合計 約"
             ...
             "受けられなくなるおそれがあります。\n"
             "※ 金額は経営戦略ベースの推計値です。一般会計繰入金はいずれのケースも同額を前提としているため、"
             "上表の差は経常損益の差として現れます。令和7年度決算では両事業とも経常損益が黒字（公共＋20,260千円・"
             "農集＋13,048千円）で、推計との突合結果は別紙1・別紙2に示しています。"),
})
...
put(SP("eval"), {
    ...
    # 20㎥までは②が最も緩やかだが、21㎥以上はすべての水量で①が最小になる。
    # 定性表記だと逆に読めてしまうため、税込の増加額を直接載せる。
    "r20": "少量使用者への配慮（10㎥・月額増）",
    "r21": "△ ＋550円", "r22": "◎ ＋220円", "r23": "○ ＋440円",
    "r30": "中量使用者の負担（30㎥・月額増）",
    "r31": "◎ ＋1,265円", "r32": "△ ＋1,595円", "r33": "○ ＋1,375円",
    "r40": "大口使用者への累進性",
...
for key, biz, sub, span in [("rec公2", "公共", "03-4", "2"), ("rec公3", "公共", "03-5", "3"),
    ...
    if biz == "公共" and span == "2":
        ...
        d["nt14"] = ("▶ 審議事項：公共下水道は今回改定のみではR9で43〜46%にとどまり、目標水準47%に到達しません。"
                     ...
                     "※ R6決算の47.11%も同一基準（汚水処理費119,503千円）です。R7の34.4%は、維持管理費が"
                     "168,331千円へ増加する見込み（＋40.9%）を織り込んだ経営戦略の推計値です。\n"
                     "※ 令和7年度決算では汚水処理費125,145千円・経費回収率45.35%となり、"
                     "推計していた維持管理費の増加は生じていません（別紙1・別紙2参照）。")
    elif biz == "公共":
...
for _k in ("rec公2", "rec公3", "rec農2", "rec農3"):
    ...
    for _r in range(5):
        for _c in range(6):
            ...
            _sp.top, _sp.height = Inches(2.13 + _r * 0.57), Inches(0.54)
            if _c in _COL:
                _rgb, _bold = _COL[_c]
