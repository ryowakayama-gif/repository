...
ORDER = [1, 2, 3, 4, 5, 23, 6, 24, 7, 8, 9, 10, 25, 11, 26, 12, 32, 13, 14,
         27, 15, 28, 16, 33, 29, 34, 17, 35, 18, 30, 19, 31, 20, 21, 22]
...
put(S_(19), {"desc": "本章では、パターン①②④それぞれの経常収支比率・経費回収率の見込みを確認し、"
...
d["note"] = ("▶ 使用料収入は②＞④＞①の順に大きく、公共下水道の経費回収率もR9で②46.1%＞④44.1%＞①43.1%。"
...
put(S_(10), {
    ...
    "eval7": "▶ 改定後の水準（税込3,300円）は青森県内平均（公共3,063円・農集2,912円）と概ね同水準。1日あたりの負担増は約29円で、住民への影響は許容範囲内と評価されます。",
})
...
for r, (lab, _) in enumerate(BLOCKS):
    d[f"d{r}0"] = lab
    for c, key in enumerate(cols3, start=1):
        d[f"d{r}{c}"] = f"{RATES[key][r]:,}"
...
for idx, biz, sub, span in [(23, "公共", "03-4", "2"), (24, "公共", "03-5", "3"),
                            (25, "農集", "03-6", "2"), (26, "農集", "03-7", "3")]:
    s = S_(idx)
    ...
    if biz == "公共" and span == "2":
        d["nt14"] = ("▶ 審議事項：公共下水道は今回改定のみではR9で43〜46%にとどまり、目標水準47%に到達しません。"
                     ...
                     "※ R7の汚水処理費168,331千円の内訳は、職員給与費5,936・動力費4,975・修繕費912・材料費1,228・"
                     "その他（主に委託料）155,280千円。\n"
                     "※ R6決算の47.11%も同一基準（汚水処理費119,503千円）です。R7の34.4%への低下は、"
                     "維持管理費が168,331千円へ増加する見込みによるものです。")
    elif biz == "公共":
        ...
        d["nt14"] = ("▶ 審議事項：農業集落排水はパターン②でR9に54.2%となり目標水準47%を達成しますが、"
                     ...
                     "※ R6決算の35.99%も同一基準（汚水処理費32,870千円）です。算定内訳はエビデンス集「汚水処理費の算定」シートに収録しています。")
    else:
...
for idx, span, sub, note in ((27, "2", "03-8", NOTE_2), (28, "3", "03-9", NOTE_3)):
    s = S_(idx)
...
# ================================================================ 30. 04-5 住民説明会
put(S_(34), {"hdr_txt": "住民への影響・説明対応　04-5　住民説明会の実施計画（案）"})
...
# 01-5 は②④①の順に並べ替えたため、パターン見出しの色を①青・②緑・④橙の凡例に合わせ直す
_one, _ = sh(S_(9))
for _n, _rgb in (("ph0", "166534"), ("ph1", "B45309"), ("ph2", "1E3A8A")):   # ②緑・④橙・①青
    _one[_n].fill.solid()
    _one[_n].fill.fore_color.rgb = RGBColor.from_string(_rgb)

# 04-4 は注記が2行になりページ番号に掛かるため、データ行の送りを詰める
_one, _ = sh(S_(33))
for _r in range(4):
    for _c in range(11):
        _sp = _one[f"d{_r}{_c}" if _c < 10 else f"d{_r}10"]
        _sp.top, _sp.height = Inches(2.14 + _r * 1.06), Inches(0.95)

# 事業所テーブルの「差額」列が元から狭く数値が折り返すため、列幅を配分し直す（04-1〜3）
for _idx in (30, 31, 32):
    _one, _ = sh(S_(_idx))
    _one["bh"].width = Inches(5.60)
    for _r in range(4):
        for _c, (_l, _w) in enumerate([(7.30, 1.95), (9.35, 1.05), (10.50, 1.15), (11.75, 1.15)]):
            _sp = _one[f"br{_r}{_c}"]
