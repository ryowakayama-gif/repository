...
d["note"] = (
    ...
    "▶ 農集は使用料収入が決算11,604千円と推計を22.5%下回る一方、一般会計繰入金は51,777千円と推計の約2.7倍でした。"
    "これは基準外繰入金を含むためで、経常収支比率が112.6%となっているのはこの繰入によるものです。"
    "※ 出所：R7公共／農集 合計残高試算表（令和8年3月31日）。")
put(s, d)
_one, _ = sh(s)
...
d = {"hb": "  参考資料　別紙2　決算を起点に組み直した場合の指標（試算）",
     ...
     "her": "経常収支比率（目標：単年度100%以上）　※（ ）内は本編の値／一般会計繰入金は経営戦略の前提（基準内繰入＝資本費相当）",
...
d["note"] = (
    ...
    "▶ 経常収支比率は一般会計繰入金の前提に左右されます。上表は経営戦略の前提（基準内繰入＝資本費相当）に"
    "よるものです。農集のR7実績51,777千円は基準外繰入金を含むため、この水準が将来も続くと置いた場合は"
    "R9で110〜111%となります。※ 有収水量・調定口数の実績が未受領のため暫定値です。")
put(s, d)
...
put(s, {
    ...
    "m01": "  • 令和8年度以降の維持管理費の見込み（流域下水道負担金63,707千円等）",
    "ch1": "引き続き確認が必要な事項",
    ...
    "rec9": "▶ 本別紙の数値は令和7年度決算（合計残高試算表・貸借対照表／令和8年3月31日、出力 令和8年6月16日）に基づきます。"
            ...
            "▶ 農集の一般会計繰入金が決算51,777千円と推計19,236千円の約2.7倍となっているのは基準外繰入金を含むためです。"
            "別紙2の試算は経営戦略の前提（基準内繰入＝資本費相当）によっています。\n"
