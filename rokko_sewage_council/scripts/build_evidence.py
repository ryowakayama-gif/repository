...
for biz in ("公共", "農集"):
    ...
    FIN = [
        ...
        ("", "補助金（一般会計繰入金）", v("補助金"), a["他会計補助金"], "農集は推計の約2.7倍（基準外繰入を含む）"),
...
for line in ("※ 有収水量・調定口数の実績は未受領のため、使用料単価・汚水処理原価は"
             ...
             "※ 農業集落排水の一般会計繰入金の実績51,777千円は基準外繰入金を含みます"
             "（資本費19,185千円の約2.7倍）。経営戦略は基準内繰入（資本費相当）のみを前提としているため、"
             "将来推計は経営戦略の前提に置いています。"):
...
for line in ("・本シートは決算突合の結果を機械的に将来へ反映した「試算」です。第2回審議会資料（37ページ）には反映していません。",
             ...
             "農業集落排水のR7実績51,777千円は基準外繰入金を含むため、資料には経営戦略の前提"
             "（基準内繰入＝資本費相当）によるケースを採用しています。"):
...
for biz in ("公共", "農集"):
    ...
    for mode, sub in (("経営戦略の前提（基準内繰入＝資本費相当）", 1.0),
                      ("R7実績水準を維持（農集は基準外繰入を含む）", s["補助金"])):
