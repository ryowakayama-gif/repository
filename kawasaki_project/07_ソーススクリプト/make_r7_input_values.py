...
def per_person(q_name, n_name, src_q="17", src_n="16"):
    """1人1月あたりの利用回（日）数 ＝ 年間回数 ÷ 年間受給者数。
    システムの入力欄は総回数ではなく1人あたりの回数である
    （システムは Σ(要介護度別人数 × 1人あたり回数) で総回数を組み立てる）。"""
    q = g17(q_name) if src_q == "17" else g19(q_name)
    n = g16(n_name) if src_n == "16" else g18(n_name)
    return [(q[i] / n[i] if n[i] else 0.0) for i in range(7)]


KAISU = [
    ("訪問介護（回）", per_person("訪問介護（回）", "訪問介護"), MASK_K),
    ("訪問入浴介護（回）", per_person("訪問入浴介護（回）", "訪問入浴介護"), MASK_ALL),
    ("訪問看護（回）", per_person("訪問看護（回）", "訪問看護"), MASK_ALL),
    ("訪問リハビリテーション（回）",
     per_person("訪問リハビリテーション（回）", "訪問リハビリテーション"), MASK_ALL),
    ("通所介護（回）", per_person("通所介護（回）", "通所介護"), MASK_K),
    ("通所リハビリテーション（回）",
     per_person("通所リハビリテーション（回）", "通所リハビリテーション"), MASK_K),
    ("短期入所生活介護（日）",
     per_person("短期入所生活介護（日）", "短期入所生活介護"), MASK_ALL),
    ("短期入所療養介護（老健）（日）",
     per_person("短期入所療養介護（介護老人保健施設）（日）",
                "短期入所療養介護（介護老人保健施設）"), MASK_ALL),
    ("短期入所療養介護（病院等）（日）",
     per_person("短期入所療養介護（病院等）（日）", "短期入所療養介護（病院等）"), MASK_ALL),
    ("短期入所療養介護（介護医療院）（日）",
     per_person("短期入所療養介護（介護医療院）（日）",
                "短期入所療養介護（介護医療院）"), MASK_ALL),
    ("地域密着型通所介護（回）",
     per_person("地域密着型通所介護（回）", "地域密着型通所介護", "19", "18"), MASK_K),
    ("認知症対応型通所介護（回）",
     per_person("認知症対応型通所介護（回）", "認知症対応型通所介護", "19", "18"), MASK_ALL),
]
...
emit("④ 在宅サービス利用回（日）数の実績見込み値の編集（令和8年度）",
     KAISU,
     "出所：年報 様式1の7(17)(19)の年間回（日）数 ÷ 様式1の7(16)(18)の年間受給者数"
     "　※総回数ではなく1人1月あたりの回（日）数")
...
CHK = [
    ("訪問介護", "人数", ZAITAKU, "訪問介護", "k"),
    ("通所介護", "人数", ZAITAKU, "通所介護", "k"),
    ("通所リハビリテーション", "人数", ZAITAKU, "通所リハビリテーション", "k"),
    ("介護予防通所リハビリテーション", "人数", ZAITAKU, "通所リハビリテーション", "y"),
    ("福祉用具貸与", "人数", ZAITAKU, "福祉用具貸与", "k"),
    ("介護予防福祉用具貸与", "人数", ZAITAKU, "福祉用具貸与", "y"),
    ("（４）居宅介護支援", "人数", ZAITAKU, "介護予防支援・居宅介護支援", "k"),
    ("（３）介護予防支援", "人数", ZAITAKU, "介護予防支援・居宅介護支援", "y"),
    ("介護老人福祉施設", "人数", SHISETSU, "介護老人福祉施設", "k"),
    ("介護老人保健施設", "人数", SHISETSU, "介護老人保健施設", "k"),
    ("認知症対応型共同生活介護", "人数", SHISETSU, "認知症対応型共同生活介護", "k"),
    ("介護予防認知症対応型共同生活介護", "人数", SHISETSU,
     "認知症対応型共同生活介護", "y"),
    ("地域密着型介護老人福祉施設入所者生活介護", "人数", SHISETSU,
     "地域密着型介護老人福祉施設入所者生活介護", "k"),
]

# 回（日）数の検算：Σ（月平均人数 × 1人1月あたり回数）が総括表の量になるか
# 総括表は予防と介護を別の行に分けているため、予防側の行名も併せて足す
KCHK = [
    ("訪問介護", "訪問介護（回）", "訪問介護", None),
    ("通所介護", "通所介護（回）", "通所介護", None),
    ("通所リハビリテーション", "通所リハビリテーション（回）", "通所リハビリテーション", None),
    ("訪問入浴介護", "訪問入浴介護（回）", "訪問入浴介護", "介護予防訪問入浴介護"),
    ("訪問看護", "訪問看護（回）", "訪問看護", "介護予防訪問看護"),
    ("訪問リハビリテーション", "訪問リハビリテーション（回）", "訪問リハビリテーション",
     "介護予防訪問リハビリテーション"),
    ("短期入所生活介護", "短期入所生活介護（日）", "短期入所生活介護",
     "介護予防短期入所生活介護"),
    ("短期入所療養介護（老健）", "短期入所療養介護（老健）（日）",
     "短期入所療養介護（老健）", "介護予防短期入所療養介護（老健）"),
    ("地域密着型通所介護", "地域密着型通所介護（回）", "地域密着型通所介護", None),
]
...
P(f"→ 人数の不一致 {ng} 件")
P()
P("■ 検算：Σ（令和7年度の月平均人数 × 1人1月あたり回数）＝ 総括表の令和7年度の量")
P("　（システムはこの式で総回数を組み立てるため、入力欄は1人1月あたりの値である）")
P(f"  {'サービス':<28}{'本書から復元':>14}{'総括表R7':>12}{'差':>10}")
ng2 = 0
for nm, kq, kn, kprev in KCHK:
    per = next((x for a, x, _ in KAISU if a == kq), None)
    ppl = next((x for a, x, _ in ZAITAKU if a == kn), None)
    if per is None or ppl is None:
        continue
    tot = sum(ppl[i] * per[i] for i in range(7))
    ref = sok.get((nm, "量"), 0) + (sok.get((kprev, "量"), 0) if kprev else 0)
    d = tot - ref
    if abs(d) > 5e-4:
        ng2 += 1
    P(f"  {nm[:28]:<28}{tot:>14.4f}{ref:>12.4f}{d:>10.4f}")
P()
P(f"→ 回（日）数の不一致 {ng2} 件")
