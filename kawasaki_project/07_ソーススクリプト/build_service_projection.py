...
def build():
    ...
    for i, v in enumerate([3187, 3160, 3133]):
        put(ws, r, 2+i, v, align="center", fmt="#,##0", fill=YELLOW)
    put(ws, r, 5, f"=SUM(B{r}:D{r})", align="center", fmt="#,##0", bold=True)
    put(ws, r, 6, "第9期計画の推計をR7年度末実績3,240人（同計画推計比−2.29%）で補正した値", size=8); r += 1
    rate_row = r
    put(ws, r, 1, "認定率", size=9)
    for i, v in enumerate([0.176, 0.180, 0.184]):
        put(ws, r, 2+i, v, align="center", fmt="0.0%", fill=YELLOW)
    put(ws, r, 5, "【入力】", align="center", size=8, fill=YELLOW)
    put(ws, r, 6, "R7年度末実績17.31%（561人÷3,240人）からの上昇。年齢階級別認定率は65-74歳3.87%／75-84歳13.22%／85歳以上58.67%", size=8); r += 1
    nin_row = r
    ...
    for lab, v, fmt, fill, memo in [
        ...
        ("調整交付金見込率", 0.039, "0.0%", YELLOW, "第9期実績3.9%。標準5%との乖離は要確認（R7年度の交付実績で確定）"),
        ("保険者機能強化交付金（千円）", 10800, "#,##0", YELLOW, "第9期実績"),
        ("第1号被保険者数 3年計（人）", f"='02_認定者数の推計'!E{pop_row}", "#,##0", GREEN, "02シートから自動参照"),
        ("予定収納率", 0.9670, "0.0%", YELLOW, "R7年度実績の総合収納率（様式3：244,747,850÷253,091,271）"),
        ("補正係数（所得段階別加重平均料率）", 1.0137, "0.0000", YELLOW,
         "R7年度末の所得段階別被保険者数（13段階・3,240人）×保険者の定める割合で実算定（年報 様式1）"),
    ]:
        put(ws, r, 1, lab, size=9)
    ...
    for i, v in enumerate([0, 39154, 78308]):
        put(ws, r, 1+i+1, v, align="center", fmt="#,##0", fill=YELLOW)
    put(ws, r, 6, "取崩なし／50%／全額（残高78,308千円＝131,700−第9期取崩予算53,392）", size=8); r += 1
    need_row = r
