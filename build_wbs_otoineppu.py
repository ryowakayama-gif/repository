...
TASKS = [
 ...
 ("1.3.4", 3, "見える化システムデータの棚卸し",
  "ダウンロード済み741ファイルを全件走査し、音威子府村の値の有無・比較群・データ年次を判定して管理表化",
  "見える化システムDLデータ棚卸し表", "受託者", "－", 1.0, D(2026,9,8), D(2026,9,18),
  "－", "3(1) 現状の評価・分析",
  "★実施済み。データは令和8年9月2〜3日出力（データあり521／全欠測130／空DL45／対象外45）"),
 ("1.3.5", 3, "未取得データの再取得・代替データの確保",
  "比較群が0団体の35ファイルを再出力。L16〜L28（在宅医療のレセプト系）は二次医療圏単位で再取得または代替を確保",
  "再取得データ／代替データ整理メモ", "受託者", "－", 1.0, D(2026,9,16), D(2026,10,15),
  "1.3.4", "3(1) 現状の評価・分析", "認知症・総合事業は村実績で補完（1.3.1で依頼）"),
 ...
 ("2.4.0", 3, "【前提】第9期計画本体の受領",
  "第9期高齢者福祉計画・介護保険事業計画（本編・資料編）および進捗管理資料を村から受領し、"
  "基本理念・基本目標・施策体系・目標値を読み取る",
  "第9期計画 構成整理メモ", "受託者", "計画の提供", 0.5, D(2026,9,8), D(2026,10,2),
  "1.1.3", "3(1)④ 現行計画の点検・評価",
  "★未受領（令和8年9月時点）。2.4.1・5.1.2・5.2.1・5.3.3 の前提となるクリティカルパス"),
 ("2.4.1", 3, "施策・事業別の進捗評価",
  "第9期計画の施策体系に沿って、事業ごとの取組状況・目標値と実績の対比・達成度（3段階等）を評価",
  "第9期計画 進捗評価シート", "受託者", "各事業の実績・自己評価", 1.5, D(2026,11,2), D(2026,11,20),
  "2.4.0, 2.2.3", "3(1)④ 現行計画の点検・評価", "村担当課への照会により評価を確定。第9期計画の受領が前提"),
 ...
 ("5.1.2", 3, "基本理念・基本目標・施策の方向の整理",
  ...
  "5.1.1, 2.4.0", "3(4)① 課題及び施策の方向",
  "★第9期の理念・目標を継承するか刷新するかの判断が必要。未受領の間は仮置きとし受領後に差替"),
...
def add_overview_sheet(wb):
    ...
    r += 1
    assumptions = [
        ("★第9期計画", "未受領。基本理念・基本目標・施策体系・第9期評価は仮置きで執筆している",
         "素案の該当箇所に『【要差替：第9期計画受領後】』を明記。受領後に一括差替（WBS 2.4.0／リスクR-13）"),
...
DELIVERABLES = [
    ...
    ("中間成果物", "計画素案（第0版・たたき台）", "電子データ（Word）", "1式",
     "令和8年9月", "5.3", "★第9期計画未受領のため理念・目標・第3章は仮置き。『【要差替】』箇所あり"),
...
RISKS = [
    ...
    ("R-11", "スコープ", "国の指針改定により推計対象年次や記載事項が追加される",
     ...
     "受託者・村", "国通知の公表時"),
    ("R-13", "スケジュール", "第9期計画（前回計画）が未受領で、基本理念・基本目標・施策体系および第9期の評価が確定できない",
     "第3章・第4章が書けず、第5章の施策構成も固まらない。素案全体が手戻りとなる", "高", "大",
     "第9期計画の提供を最優先で依頼（WBS 2.4.0）。受領までは仮置きで執筆し、該当箇所に『【要差替：第9期計画受領後】』"
     "を明記して機械的に抽出できるようにする。受領後に一括差替",
     "受託者・村", "毎週／受領まで"),
    ("R-14", "品質", "見える化システムの在宅医療系（L16〜L28）90ファイルが空振りで、認知症・総合事業も欠測または更新停止",
     "第5章（在宅医療・介護連携、認知症施策、介護予防）の現状記述の根拠が不足", "高", "中",
     "二次医療圏単位での再取得または北海道の医療計画等で代替。認知症・総合事業は村実績で補完（WBS 1.3.5）",
     "受託者・村", "第1回打合せ／毎月"),
    ("R-15", "品質", "見える化システムの35ファイルが音威子府村のみの出力で管内他団体と比較できない",
     "近隣比較の図表が作れず、保険料水準や認定率の位置づけを説明できない", "中", "中",
     "比較地域を指定して再出力（WBS 1.3.5）。特にC1（保険料）は議会・住民説明で必要",
     "受託者", "令和8年9月中"),
